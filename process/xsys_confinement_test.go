...
// syscall. The package-wide import allowlist admits that
...
// file imports syscall, and when a named leaf stops
...
func TestSubstrateEscapeStaysInNamedLeafFiles(t *testing.T) {
	...
	for _, entry := range entries {
		...
		for _, imported := range parsed.Imports {
			...
			if strings.HasPrefix(path, "golang.org/x/sys") {
				t.Errorf("production file %s imports %s; use the Go standard library", name, path)
			}
			if path != "syscall" {
				continue
			}
	...
	for name := range allowed {
		if !seen[name] {
			t.Errorf("named platform leaf %s no longer imports syscall, want the inventory row removed with the escape", name)
		}
	}
}
