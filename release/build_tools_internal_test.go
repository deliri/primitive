...
func TestParseGoVersionOutputPressuresEverySideOfTheProbeGrammar(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "three fields is rejected", output: "go version " + want + "\n", wantErr: core.ErrReleaseContract},
		{name: "five fields is rejected", output: "go version " + want + " linux/amd64 extra" + "\n", wantErr: core.ErrReleaseContract},
		...
		{name: "empty version token is rejected", output: "go version  linux/amd64" + "\n", wantErr: core.ErrReleaseContract},
		{name: "platform without a separator is rejected", output: "go version " + want + " linuxamd64" + "\n", wantErr: core.ErrReleaseContract},
		{name: "platform with two separators is rejected", output: "go version " + want + " linux/amd64/v3" + "\n", wantErr: core.ErrReleaseContract},
		{name: "platform with a trailing separator is rejected", output: "go version " + want + " linux/" + "\n", wantErr: core.ErrReleaseContract},
		{name: "platform with a leading separator is rejected", output: "go version " + want + " /amd64" + "\n", wantErr: core.ErrReleaseContract},
		{name: "unknown operating system is rejected", output: "go version " + want + " plan9/amd64" + "\n", wantErr: core.ErrReleaseContract},
		{name: "unknown architecture is rejected", output: "go version " + want + " linux/riscv64" + "\n", wantErr: core.ErrReleaseContract},
		{name: "already hyphenated platform is rejected", output: "go version " + want + " linux-amd64" + "\n", wantErr: core.ErrReleaseContract},
		{name: "uppercase platform is rejected", output: "go version " + want + " Linux/AMD64" + "\n", wantErr: core.ErrReleaseContract},
		{name: "oversized trailing field is rejected", output: "go version " + want + " " + strings.Repeat("a", buildToolProbeOutputBytes) + "\n", wantErr: core.ErrReleaseContract},
		{name: "embedded NUL platform is rejected", output: "go version " + want + " linux/amd64\x00" + "\n", wantErr: core.ErrReleaseContract},
	}
