...
func FuzzTopicNameGrammar(f *testing.F) {
	...
	for _, seed := range []string{"compile", "work.list", "anvil.file.run", "work-item.read-all", "work..list", "work.-list", "work-.list", "Work.list", "", "x\n"} {
		f.Add(seed)
	}
	f.Fuzz(func(t *testing.T, text string) {
		want := len(text) <= manual.MaximumTopicBytes && grammar.MatchString(text)
