...
func parseAmazonQueryField(value string) (amazonQueryField, error) {
	...
	return amazonQueryFieldUnknown, core.ErrAWSIdentityContract
}
...
func Acquire(
	...
	if err != nil {
		return Token{}, requestFailure(err)
	}
...
func exactAmazonQueryDomain(query url.Values) bool {
	if len(query) < amazonQueryFieldCount-1 || len(query) > amazonQueryFieldCount {
		return false
	}
