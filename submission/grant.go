...
const (
	// AuthorizationNonceBytes is the exact width of one authority grant nonce.
	AuthorizationNonceBytes = 32
...
// AuthorizationNonce is one public unpredictable authority decision identity.
type AuthorizationNonce struct {
	value core.SHA256Digest
}

// NewAuthorizationNonce owns exact nonzero authority-generated bytes.
func NewAuthorizationNonce(value [AuthorizationNonceBytes]byte) (AuthorizationNonce, error) {
	candidate := AuthorizationNonce{value: core.NewSHA256Digest(value)}
	if err := candidate.Validate(); err != nil {
		return AuthorizationNonce{}, err
	}
	return candidate, nil
}

// ParseAuthorizationNonce accepts canonical lowercase hexadecimal.
func ParseAuthorizationNonce(value string) (AuthorizationNonce, error) {
	if len(value) != hex.EncodedLen(AuthorizationNonceBytes) {
		return AuthorizationNonce{}, contractError(errors.New("authorization nonce has invalid extent"))
	}
	var raw [AuthorizationNonceBytes]byte
	if err := core.DecodeCanonicalHex(raw[:], value); err != nil {
		return AuthorizationNonce{}, contractError(err)
	}
	return NewAuthorizationNonce(raw)
}

// Validate rejects the unset or all-zero nonce.
func (n AuthorizationNonce) Validate() error {
	return validateNonzeroDigest(n.value, "authorization nonce is all zero")
}

// String returns canonical lowercase hexadecimal, or empty text when unset.
func (n AuthorizationNonce) String() string {
	if n.Validate() != nil {
		return ""
	}
...
func (n AuthorizationNonce) MarshalJSON() ([]byte, error) {
	if err := n.Validate(); err != nil {
		return nil, jsonError(err)
	}
	return core.MarshalCanonicalJSONString(n.String())
}

// UnmarshalJSON accepts one canonical nonce without mutating on refusal.
func (n *AuthorizationNonce) UnmarshalJSON(data []byte) error {
	if n == nil {
		return jsonError(errors.New("nil authorization nonce receiver"))
	}
	value, err := core.DecodeJSONStringToken(data)
	if err != nil {
		return jsonError(err)
	}
	candidate, err := ParseAuthorizationNonce(value)
	if err != nil {
		return jsonError(err)
	}
	*n = candidate
...
type GrantPayload struct {
	...
	Authorization AuthorizationNonce                     `json:"authorization_nonce"`
...
var (
	_ core.Validatable = AuthorizationNonce{}
	...
	_ core.ValidatedJSONMarshaler         = AuthorizationNonce{}
	_ core.ValidatedJSONMarshaler         = GrantPayload{}
	_ core.ValidatedJSONMarshaler         = GrantProjection{}
	_ json.Unmarshaler                    = (*AuthorizationNonce)(nil)
