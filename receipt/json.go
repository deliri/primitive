...
func (c jsonStructureContract) limits() (core.StrictJSONLimits, error) {
	checked, err := core.CheckedUint32FromInt(c.maximumBytes)
	if err != nil {
		return core.StrictJSONLimits{}, jsonError(err)
	}
	count, err := core.NewByteCount(uint64(checked))
	if err != nil {
		return core.StrictJSONLimits{}, jsonError(err)
	}
	limits := core.StrictJSONLimits{
		DocumentMaximumBytes: count,
...
func writeCanonical(destination io.Writer, data []byte) error {
	if destination == nil {
		return contractError(errors.New("receipt canonical destination is nil"))
	}
