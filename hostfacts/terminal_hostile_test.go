...
func TestTerminalColumnsAdmitOnlyUsableWidths(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "zero columns are rejected", columns: 0, wantErr: true},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if gotErr := err != nil; gotErr != tc.wantErr {
				t.Fatalf("TerminalColumns(%d).Validate() error = %v, wantErr %t", tc.columns, err, tc.wantErr)
			}
			if tc.wantErr && !errors.Is(err, core.ErrHostFactsContract) {
				t.Fatalf("TerminalColumns(%d).Validate() error = %v, want %v", tc.columns, err, core.ErrHostFactsContract)
			}
			if got, want := tc.columns.IsValid(), !tc.wantErr; got != want {
...
func TestTerminalGeometryValidatesAttachmentAgainstColumns(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "unset attachment describes nothing", geometry: TerminalGeometry{}, wantErr: true},
		{name: "unset attachment with columns is still nothing", geometry: TerminalGeometry{columns: 80}, wantErr: true},
		{name: "attachment beyond the closed domain is rejected", geometry: TerminalGeometry{attachment: terminalAttachmentLimit}, wantErr: true},
		{name: "attachment far beyond the closed domain is rejected", geometry: TerminalGeometry{attachment: TerminalAttachment(math.MaxUint8)}, wantErr: true},
		{name: "a terminal with zero columns is a contradiction", geometry: TerminalGeometry{attachment: TerminalAttachmentTerminal}, wantErr: true},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if gotErr := err != nil; gotErr != tc.wantErr {
				t.Fatalf("TerminalGeometry.Validate() error = %v, wantErr %t", err, tc.wantErr)
			}
			if !tc.wantErr {
				return
			}
			if !errors.Is(err, core.ErrHostFactsContract) {
				t.Fatalf("TerminalGeometry.Validate() error = %v, want %v", err, core.ErrHostFactsContract)
			}
			if attachment, accessErr := tc.geometry.Attachment(); accessErr == nil || attachment != TerminalAttachmentUnknown {
				t.Fatalf("invalid geometry Attachment() = (%v, %v), want (%v, contract refusal)", attachment, accessErr, TerminalAttachmentUnknown)
			}
			if columns, accessErr := tc.geometry.Columns(); accessErr == nil || columns != 0 {
