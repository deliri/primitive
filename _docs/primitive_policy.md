...
| 7 | `submissionauth` | Installation-certificate binding, device authentication, and authority reconciliation for evidence submissions | `core`, `attest`, `chit`, `controlplane`, `controlwire`, `objectstore`, `receipt`, `submission` | `controlplanetest` |
...
| 6 | `gcsobjects` | Authenticated Google Cloud Storage bucket provisioning, typed logical namespace composition, create-only writes, exact-generation observation, digest-bound reads, and generation-matched permanent deletion through the official SDK | `core`, `contextstate`, `temporal`, `objectstore` | `exchange`, `testserial` |
