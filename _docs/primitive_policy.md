...
| 6 | `gcsobjects` | Authenticated Google Cloud Storage bucket provisioning, typed logical namespace composition, create-only writes, digest-bound reads, and generation-matched permanent deletion through the official SDK | `core`, `contextstate`, `temporal`, `objectstore` | none |
