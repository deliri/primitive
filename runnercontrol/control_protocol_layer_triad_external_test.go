...
func TestAdmissionSchemaLayerTriad(t *testing.T) {
	...
	t.Run("positive Go file selection retains its exact package file and child policy", func(t *testing.T) {
		t.Parallel()
		authenticated, admitted := admissionFixture(t)
		module, moduleErr := runprotocol.NewIdentifier("blink-kernel")
		packagePath, packageErr := runprotocol.ParseSourcePath("api")
		file, fileErr := runprotocol.ParseSourcePath("api/api_test.go")
		if err := errors.Join(moduleErr, packageErr, fileErr); err != nil {
			t.Fatalf("Go file selection target setup error = %v, want nil", err)
		}
		target := runprotocol.ProbeTarget{Kind: runprotocol.ProbeTargetGoFile, GoFile: &runprotocol.GoFileTarget{Module: module, Package: packagePath, File: file, ChildKinds: []runprotocol.ProbeKind{runprotocol.ProbeKindGoTest}}}
		authenticated.Requested.Probe.Target = target
		authenticated.Requested.Probe.Kinds = []runprotocol.ProbeKind{runprotocol.ProbeKindGoTest}
		probe, probeErr := runprotocol.AdmitRequestedProbe(authenticated.Requested.Probe, admitted.Probe.Environment)
		...
		if admitted.Probe.Kind != runprotocol.ProbeKindGoFileSelection || admitted.Probe.Target.GoFile == nil || admitted.Probe.Target.GoFile.File != file {
	...
	t.Run("negative authenticated origin cannot claim another origin request", func(t *testing.T) {
		...
		foreign := runprotocol.OriginIdentity{Offering: core.Offering{Token: "forge"}}
	...
	t.Run("negative request identity cannot detach from its caller nonce and origin", func(t *testing.T) {
		t.Parallel()
		authenticated, _ := admissionFixture(t)
		foreignOrigin := runprotocol.OriginIdentity{Offering: core.Offering{Token: "foreign-origin"}}
		foreignRequest, foreignErr := runprotocol.DeriveRequestIdentity(foreignOrigin, authenticated.Requested.Nonce)
	...
	t.Run("neutral typed refusal carries no admitted run", func(t *testing.T) {
		t.Parallel()
		authenticated, _ := admissionFixture(t)
		refusal := runprotocol.RefusalUnauthorized
...
func admissionFixture(t testing.TB) (runnercontrol.AuthenticatedAdmissionRequest, runnercontrol.AdmittedRun) {
	...
	nonce, nonceErr := runprotocol.NewRequestNonce(completionUUIDFixture(t))
	request, requestErr := runprotocol.DeriveRequestIdentity(completion.Probe.Origin, nonce)
	...
	audience, audienceErr := runprotocol.NewIdentifier("origin-runner")
	application, applicationErr := runprotocol.NewIdentifier("runner")
	credential, custodyErr := runprotocol.NewIdentifier("source-read-once")
	...
	requestedProbe := runprotocol.RequestedProbe{
		Origin: completion.Probe.Origin, Subject: completion.Probe.Subject, Source: completion.Probe.Source,
		Target: completion.Probe.Target, Kinds: []runprotocol.ProbeKind{completion.Probe.Kind}, Profile: completion.Probe.Profile,
		Constraints: runprotocol.EnvironmentRequirement{MachineClass: completion.Probe.Environment.MachineClass, Fingerprint: completion.Probe.Environment.RequirementFingerprint},
	}
...
func artifactFixture(t testing.TB, data []byte) (runnercontrol.ArtifactManifest, runnercontrol.ArtifactChunk) {
	...
	path, pathErr := runprotocol.ParseSourcePath("experiments/stdout.jsonl")
