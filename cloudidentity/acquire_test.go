...
func TestAmazonResponseXMLPressureTable(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "declared prefix namespace is accepted", body: `<sts:GetWebIdentityTokenResponse xmlns:sts="` + amazonResponseNamespace + `"><sts:GetWebIdentityTokenResult><sts:WebIdentityToken>a.b.c</sts:WebIdentityToken><sts:Expiration>2026-07-29T12:05:00Z</sts:Expiration></sts:GetWebIdentityTokenResult><sts:ResponseMetadata><sts:RequestId>request-id</sts:RequestId></sts:ResponseMetadata></sts:GetWebIdentityTokenResponse>`, wantToken: "a.b.c"},
		...
		{name: "missing documented expiration is rejected", body: strings.Replace(envelope, "<Expiration>2026-07-29T12:05:00Z</Expiration>", "", 1), wantErr: core.ErrCloudIdentityContract},
		{name: "duplicated documented expiration is rejected", body: strings.Replace(envelope, "</GetWebIdentityTokenResult>", "<Expiration>2026-07-29T12:05:01Z</Expiration></GetWebIdentityTokenResult>", 1), wantErr: core.ErrCloudIdentityContract},
		{name: "malformed documented expiration is rejected", body: strings.Replace(envelope, "2026-07-29T12:05:00Z", "not-a-timestamp", 1), wantErr: core.ErrCloudIdentityContract},
		{name: "non-UTC documented expiration is rejected", body: strings.Replace(envelope, "2026-07-29T12:05:00Z", "2026-07-29T13:05:00+01:00", 1), wantErr: core.ErrCloudIdentityContract},
		{name: "expiration switching to a foreign namespace is rejected", body: strings.Replace(envelope, "<Expiration>", `<Expiration xmlns="https://evil.example.com/">`, 1), wantErr: core.ErrCloudIdentityContract},
		{name: "nested markup inside expiration is rejected", body: strings.Replace(envelope, "2026-07-29T12:05:00Z", "2026-07-29<Ignored/>T12:05:00Z", 1), wantErr: core.ErrCloudIdentityContract},
		{name: "unknown result member is rejected", body: strings.Replace(envelope, "</GetWebIdentityTokenResult>", "<Unknown>value</Unknown></GetWebIdentityTokenResult>", 1), wantErr: core.ErrCloudIdentityContract},
		{name: "missing response metadata is rejected", body: strings.Replace(envelope, "<ResponseMetadata><RequestId>request-id</RequestId></ResponseMetadata>", "", 1), wantErr: core.ErrCloudIdentityContract},
		{name: "duplicated response metadata is rejected", body: strings.Replace(envelope, "</GetWebIdentityTokenResponse>", "<ResponseMetadata><RequestId>second-id</RequestId></ResponseMetadata></GetWebIdentityTokenResponse>", 1), wantErr: core.ErrCloudIdentityContract},
		{name: "missing response request identity is rejected", body: strings.Replace(envelope, "<RequestId>request-id</RequestId>", "", 1), wantErr: core.ErrCloudIdentityContract},
		{name: "nested markup inside response request identity is rejected", body: strings.Replace(envelope, "request-id", "request<Ignored/>-id", 1), wantErr: core.ErrCloudIdentityContract},
		{name: "unknown root member is rejected", body: strings.Replace(envelope, "</GetWebIdentityTokenResponse>", "<Unknown>value</Unknown></GetWebIdentityTokenResponse>", 1), wantErr: core.ErrCloudIdentityContract},
