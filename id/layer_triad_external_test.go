...
func TestIdentityConstructionAndParsingLayerTriad(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "negative noncanonical identities return typed zero values", run: func(t *testing.T) {
			t.Parallel()
		...
		{name: "neutral zero identities project no plausible text or JSON", run: func(t *testing.T) {
			t.Parallel()
			var uuid id.UUIDv7
			var ulid id.ULID
