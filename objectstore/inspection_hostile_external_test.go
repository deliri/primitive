...
func TestInspectStreamsExactIntegrityAcrossHostileChunkBoundaries(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if reader.position != tc.size || got.Validate() != nil || got.Integrity.Length.Uint64() != uint64(tc.size) || got.Integrity.SHA256 != wantSHA || got.BLAKE3 != wantBLAKE3 || got.Integrity.CRC32C != wantCRC {
...
func canceledInspectionContext(t testing.TB) context.Context {
...
func expiredInspectionContext(t testing.TB) context.Context {
	...
	ctx, cancel, err := temporal.WithDeadline(temporal.DeadlineRequest{Parent: t.Context(), Deadline: temporal.InstantFromNanoseconds(1)})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(cancel)
