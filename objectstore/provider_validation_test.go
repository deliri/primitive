...
func TestDownloadProviderValidationHostileBoundaryTable(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "S3 one below single GetObject maximum is accepted", provider: ProviderAmazonS3, mutate: setDownloadLength(AmazonS3SingleGetObjectMaximumBytes - 1)},
		{name: "S3 exact single GetObject maximum is accepted", provider: ProviderAmazonS3, mutate: setDownloadLength(AmazonS3SingleGetObjectMaximumBytes)},
		{name: "S3 one above single GetObject maximum is rejected", provider: ProviderAmazonS3, mutate: setDownloadLength(AmazonS3SingleGetObjectMaximumBytes + 1), wantErr: core.ErrObjectStoreSize},
