...
func TestHTTPEndpointHostileBoundaryTable(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "empty fragment marker is rejected", value: "https://example.test/path#", wantErr: ErrPrimitiveContract},
		{name: "zero port is rejected", value: "https://example.test:0", wantErr: ErrPrimitiveContract},
		{name: "leading-zero default port is rejected", value: "http://example.test:080", wantErr: ErrPrimitiveContract},
		{name: "leading-zero nondefault port is rejected", value: "https://example.test:08443", wantErr: ErrPrimitiveContract},
