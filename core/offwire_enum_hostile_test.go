...
type coreOffWireEnum interface {
	comparable
	OffWireEnum
	IsValid() bool
	String() string
...
func TestCoreOffWireEnumsExhaustClosedDomains(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{name: "test isolation hazards reject every unadmitted uint8 value", run: func(t *testing.T) {
			proveCoreOffWireEnum(t, func(raw uint8) TestIsolationHazard { return TestIsolationHazard(raw) }, []TestIsolationHazard{
		...
		{name: "test isolation scopes reject every unadmitted uint8 value", run: func(t *testing.T) {
			proveCoreOffWireEnum(t, func(raw uint8) TestIsolationScope { return TestIsolationScope(raw) }, []TestIsolationScope{
...
func proveCoreOffWireEnum[T coreOffWireEnum](
	t *testing.T,
	...
	proveCoreEnumStaysOffWire(t, admitted[0])
}

func proveCoreEnumStaysOffWire[T coreOffWireEnum](t *testing.T, value T) {
	t.Helper()
