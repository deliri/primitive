...
func TestDecodeCanonicalHexAdmitsOnlyTheOneCanonicalSpelling(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "uppercase is not canonical", destinationSize: 1, value: "0A", wantErr: core.ErrPrimitiveContract},
		{name: "mixed case is not canonical", destinationSize: 2, value: "aBcd", wantErr: core.ErrPrimitiveContract},
		{name: "text one byte short is the wrong extent", destinationSize: 2, value: "ab", wantErr: core.ErrPrimitiveContract},
		{name: "text one byte long is the wrong extent", destinationSize: 1, value: "abcd", wantErr: core.ErrPrimitiveContract},
		{name: "odd length text can never be byte exact", destinationSize: 1, value: "abc", wantErr: core.ErrPrimitiveContract},
		{name: "empty text cannot fill a nonempty destination", destinationSize: 1, value: "", wantErr: core.ErrPrimitiveContract},
		{name: "a 0x prefix is a different language", destinationSize: 2, value: "0xab", wantErr: core.ErrPrimitiveContract},
		{name: "letters beyond f only look like hex", destinationSize: 1, value: "zz", wantErr: core.ErrPrimitiveContract},
		{name: "interior whitespace is refused", destinationSize: 2, value: "ab cd", wantErr: core.ErrPrimitiveContract},
		{name: "unicode digits that render like hex are refused", destinationSize: 2, value: "abсd", wantErr: core.ErrPrimitiveContract},
	}
