...
func TestSignedPaymentQueryLayerTriad(t *testing.T) {
	t.Parallel()
	...
	t.Run("negative missing conflicting future and foreign facts are refused", func(t *testing.T) {
		...
		cases := []struct {
			...
			{name: "zero verification", mutate: func(value *QueryVerification) { *value = QueryVerification{} }, want: core.ErrPaymentContract},
			{name: "query document absent", mutate: func(value *QueryVerification) { value.Document = QueryDocument{} }, want: core.ErrPaymentContract},
			...
			{name: "selection changed after signing", mutate: func(value *QueryVerification) { value.Document.Payload.Query.Selection = signedQuerySpecific(t, 0x62) }, want: core.ErrPaymentVerification},
			{name: "position changed after signing", mutate: func(value *QueryVerification) { value.Document.Payload.Query.Position = signedQueryAfter(t, 0x63) }, want: core.ErrPaymentVerification},
			{name: "page limit changed after signing", mutate: func(value *QueryVerification) { value.Document.Payload.Query.Limit = signedQueryLimit(t, 2) }, want: core.ErrPaymentVerification},
			{name: "build changed after signing", mutate: func(value *QueryVerification) { value.Document.Payload.Build = other.payload.Build }, want: core.ErrPaymentVerification},
			{name: "nonce changed after signing", mutate: func(value *QueryVerification) { value.Document.Payload.Nonce = other.payload.Nonce }, want: core.ErrPaymentVerification},
			{name: "revision changed to future value", mutate: func(value *QueryVerification) { value.Document.Payload.Revision = controlwire.Revision(255) }, want: core.ErrPaymentContract},
			{name: "attestation domain changed to payment receipt", mutate: func(value *QueryVerification) { value.Document.Attestation.Domain = SigningDomainReceiptV1 }, want: core.ErrPaymentVerification},
			{name: "attestation domain changed to catalog response", mutate: func(value *QueryVerification) { value.Document.Attestation.Domain = SigningDomainCatalogV1 }, want: core.ErrPaymentVerification},
		}
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				t.Parallel()
	...
	t.Run("neutral zero issuance creates no signed query", func(t *testing.T) {
		...
		if !errors.Is(err, core.ErrPaymentContract) || got != (QueryDocument{}) {
			t.Fatalf("IssueQuery(zero) = (%+v, %v), want zero and errors.Is %v",
				got, err, core.ErrPaymentContract)
		}
	})
}
...
func TestPaymentSigningDomainExhaustsPublishedAndFutureByteValues(t *testing.T) {
	t.Parallel()
		...
		if !domain.IsValid() {
			if err := domain.Validate(); !errors.Is(err, core.ErrPaymentContract) {
				t.Fatalf("SigningDomain(%d).Validate() error = %v, want errors.Is %v",
					value, err, core.ErrPaymentContract)
			}
			continue
		}
		admitted++
...
func newSignedQueryFixture(t testing.TB, request signedQueryFixtureRequest) signedQueryFixture {
	...
	query, err := NewQuery(QueryRequest{
		Scope: paymentScopeFixture(t, request.marker), Selection: request.selection,
	...
	private, trusted := paymentSigningFixture(t, request.marker+0x40)
...
func signedQuerySpecific(t testing.TB, marker byte) Selection {
	...
	selection, err := Specific(mustPaymentID(t, marker, int64(marker)+10))
	if err != nil {
