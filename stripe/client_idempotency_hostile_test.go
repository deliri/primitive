...
func TestIdempotencyHeaderCrossesStripeWallAtPublishedMaximum(t *testing.T) {
	t.Parallel()

	wantKey := strings.Repeat("s", core.StripeIdempotencyKeyMaximumBytes)
	var gotKey string
	var calls uint64
	...
	credential, err := ParseCredential([]byte("rk_test_credential"))
	if err != nil {
	...
	request := stripeIdempotencyRequest(t, wantKey)
	...
	if gotErr := (Request{Stream: request}).Validate(); gotErr != nil {
		t.Fatalf("Stripe Request.Validate() error = %v, want nil", gotErr)
	}
	policy := stripeStreamPolicy(t)
...
func stripeIdempotencyRequest(t testing.TB, keyText string) exchange.StreamRoundTripRequest {
	t.Helper()
	target, err := core.ParseHTTPEndpoint("https://" + core.StripeAPIHost + "/v1/payment_intents")
	if err != nil {
		t.Fatalf("core.ParseHTTPEndpoint() error = %v, want nil", err)
	}
	requestType, err := core.ParseHTTPMediaType("application/x-www-form-urlencoded")
	if err != nil {
		t.Fatalf("core.ParseHTTPMediaType(request) error = %v, want nil", err)
	}
	responseType, err := core.ParseHTTPMediaType("application/json")
	if err != nil {
...
func stripeStreamPolicy(t testing.TB) exchange.StreamPolicy {
	t.Helper()
