...
| `objectstore` | Exact bounded S3, GCS, or Cloudflare Images transfers through already-issued HTTPS capabilities, plus authenticated GCS create-only write, exact read, and generation-matched permanent prefix deletion through the official SDK. Typed bucket/object/prefix/cache/generation contracts, dual integrity, standard object time metadata, capability commitments, and post-delete absence proof. |
...
| authenticated GCS object create/read/permanent prefix delete | `objectstore` — use `NewGCSClient`; products never import the provider SDK |
