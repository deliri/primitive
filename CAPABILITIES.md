...
| `submission` | Device-signed evidence declarations, portable `ManifestIntent` facts (opaque upload UUIDv7, collection, customer-visible name, sequence and total), and authority upload-or-reuse decisions. Grants bind one exact request, upload-capability commitment, authority nonce, expiry, and retention promise; reuse carries already-authenticated accepted-object evidence. A verified upload decision projects the exact `UploadCapabilityRequest` for Objectstore without deciding product policy or executing the transfer itself. |
