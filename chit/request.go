...
const (
	...
	queryCommitmentDomain              = "primitive/chit/query-commitment/v1"
...
func (p *QueryPayload) UnmarshalJSON(data []byte) error {
	if p == nil {
		return jsonError(errors.New("nil chit query payload receiver"))
	}
	type wire QueryPayload
...
func (c QueryCommitment) Validate() error {
	...
	return contractError(errors.New("chit query commitment is all zero"))
}
...
func (d QueryDocument) Validate() error {
	...
	if d.Attestation.Domain != SigningDomainQueryV1 {
		return verificationError(errors.New("chit query signing domain differs"))
	}
	return nil
}
...
func (d *QueryDocument) UnmarshalJSON(data []byte) error {
	if d == nil {
		return jsonError(errors.New("nil chit query document receiver"))
	}
	type wire QueryDocument
