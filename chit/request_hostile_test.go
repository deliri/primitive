...
func TestSignedChitQueryLayerTriad(t *testing.T) {
	...
	t.Run("positive exact all specific page and product boundaries authenticate", func(t *testing.T) {
		...
		cases := []struct {
			...
			{name: "all minimum page witness", request: signedQueryFixtureRequest{marker: 0x21, offering: chitOffering(t, 2), pageSize: 1}, wantKind: core.CatalogSelectionAll, wantLimit: 1},
			{name: "all one above minimum page bug", request: signedQueryFixtureRequest{marker: 0x22, offering: chitOffering(t, 1), pageSize: 2}, wantKind: core.CatalogSelectionAll, wantLimit: 2},
			{name: "all midpoint page peachfuzz", request: signedQueryFixtureRequest{marker: 0x23, offering: chitOffering(t, 3), pageSize: core.CatalogPageMaximumEntries / 2}, wantKind: core.CatalogSelectionAll, wantLimit: core.CatalogPageMaximumEntries / 2},
