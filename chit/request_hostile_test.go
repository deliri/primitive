...
func TestSignedChitQueryLayerTriad(t *testing.T) {
	...
	t.Run("positive exact all specific page and product boundaries authenticate", func(t *testing.T) {
		...
		cases := []struct {
			...
			{name: "all minimum page product beta", request: signedQueryFixtureRequest{marker: 0x21, offering: chitOffering(t, 2), pageSize: 1}, wantKind: core.CatalogSelectionAll, wantLimit: 1},
			{name: "all one above minimum page product alpha", request: signedQueryFixtureRequest{marker: 0x22, offering: chitOffering(t, 1), pageSize: 2}, wantKind: core.CatalogSelectionAll, wantLimit: 2},
			{name: "all midpoint page product gamma", request: signedQueryFixtureRequest{marker: 0x23, offering: chitOffering(t, 3), pageSize: core.CatalogPageMaximumEntries / 2}, wantKind: core.CatalogSelectionAll, wantLimit: core.CatalogPageMaximumEntries / 2},
	...
	t.Run("negative missing conflicting future and foreign facts are refused", func(t *testing.T) {
		...
		other := newSignedQueryFixture(t, signedQueryFixtureRequest{
			marker: 0x61, offering: chitOffering(t, 1), pageSize: 2,
		})
		cases := []struct {
			want   error
...
func TestQueryCommitmentBindsEveryVariableRequestFact(t *testing.T) {
	...
	other := newSignedQueryFixture(t, signedQueryFixtureRequest{
		marker: 0x71, offering: chitOffering(t, 1), pageSize: 2,
	})
	cases := []struct {
...
func newSignedQueryFixture(t testing.TB, request signedQueryFixtureRequest) signedQueryFixture {
	...
	if request.offering == (core.Offering{}) {
		request.offering = chitOffering(t, 2)
	}
