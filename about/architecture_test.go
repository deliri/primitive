...
type aboutJSONDoorInventory struct {
...
var aboutJSONDoors = aboutJSONDoorInventory{
...
func TestAboutDataFlowStructInventoryRatchet(t *testing.T) {
	t.Parallel()

	gotStructs, gotMarkers, gotErr := scanAboutDataFlowInventory()
	if gotErr != nil {
		t.Fatalf("scanAboutDataFlowInventory() error = %v, want nil", gotErr)
	}
	if !slices.Equal(gotStructs, gotMarkers) {
		t.Fatalf("About production structs = %q, want exactly one classified marker each; markers = %q", gotStructs, gotMarkers)
	}
}

func TestAboutExternalIngressInventoryMatchesSemanticFuzzDoors(t *testing.T) {
	t.Parallel()

	gotJSON, gotText, gotErr := scanAboutExternalDoors()
	if gotErr != nil {
		t.Fatalf("scanAboutExternalDoors() error = %v, want nil", gotErr)
	}
	wantJSON := inventoryFieldNames(aboutJSONDoors)
	wantText := inventoryFieldNames(aboutTextDoors)
	if !slices.Equal(gotJSON, wantJSON) {
		t.Fatalf("About production JSON doors = %q, want semantic fuzz inventory %q", gotJSON, wantJSON)
	}
	if !slices.Equal(gotText, wantText) {
		t.Fatalf("About production text doors = %q, want semantic fuzz inventory %q", gotText, wantText)
	}
	if len(wantJSON) != int(aboutJSONDoorLimit)+5 {
		t.Fatalf("About JSON fuzz selectors = %d atomic + 5 documents, want %d inventoried doors", aboutJSONDoorLimit, len(wantJSON))
	}
}

func scanAboutDataFlowInventory() ([]string, []string, error) {
...
func scanAboutExternalDoors() ([]string, []string, error) {
	entries, err := aboutContractSources.ReadDir(".")
	if err != nil {
		return nil, nil, err
	}
...
func aboutRoleMarker(name string) bool {
	switch name {
	case "aboutProtocolFact", "aboutSealedProjection", "aboutInternalFlowCarrier", "aboutCapabilityWrapper":
		return true
	default:
		return false
	}
