...
// QueryKind is the closed About read domain.
...
func (k QueryKind) Validate() error {
	return validateEnum(uint8(k), queryKindLabels(), "about query kind is invalid")
}
...
func (k QueryKind) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(k), queryKindLabels(), "about query kind is invalid")
}

func (k *QueryKind) UnmarshalJSON(data []byte) error {
	if k == nil {
		return jsonError(errors.New("nil about query kind receiver"))
	}
	value, err := unmarshalEnum(data, queryKindLabels(), "about query kind is invalid")
	if err == nil {
		*k = QueryKind(value)
	}
	return err
...
func (q Query) Validate() error {
	if q.SchemaVersion != SchemaVersion {
		return contractError(errors.New("about query schema version is unsupported"))
	}
	...
	return conflictError(errors.New("about query payload differs from kind"))
}
...
func (q *Query) UnmarshalJSON(data []byte) error {
	if q == nil {
		return jsonError(errors.New("nil about query receiver"))
	}
...
func (r Response) Validate() error {
	if r.SchemaVersion != SchemaVersion {
		return contractError(errors.New("about response schema version is unsupported"))
	}
	if err := r.Query.Validate(); err != nil {
		return err
	}
	if (r.Project != nil) == (r.Package != nil) {
		return contractError(errors.New("about response must carry exactly one projection"))
	}
	if r.Project != nil {
...
func (r Response) validateProject() error {
	if r.Query.Kind != QueryProject {
		return conflictError(errors.New("about project response differs from query kind"))
	}
	if err := r.Project.Validate(); err != nil {
		return err
	}
	if !sameSubject(r.Project.Subject, r.Query.Subject) || r.Project.Revision != r.Query.Revision {
		return conflictError(errors.New("about project response differs from query source"))
	}
	return nil
}
...
func (r Response) validatePackage() error {
	if r.Query.Kind != QueryPackage || r.Query.Package == nil {
		return conflictError(errors.New("about package response differs from query kind"))
	}
	...
	if !sameSubject(r.Package.Package.Subject, r.Query.Subject) || r.Package.Package.Revision != r.Query.Revision || r.Package.Package.Knowledge.Path != *r.Query.Package {
		return conflictError(errors.New("about package response differs from query source"))
	}
	return nil
}
...
func (r *Response) UnmarshalJSON(data []byte) error {
	if r == nil {
		return jsonError(errors.New("nil about response receiver"))
	}
...
// NewService constructs one About query service.
func NewService(repository Repository) (Service, error) {
	if repository == nil {
		return Service{}, contractError(errors.New("about repository is nil"))
	}
...
func (s Service) Resolve(ctx context.Context, query Query) (Response, error) {
	if s.repository == nil {
		return Response{}, contractError(errors.New("about service is unconstructed"))
	}
...
func contextTerminal(ctx context.Context) error {
	if ctx == nil {
		return contractError(errors.New("about context is nil"))
	}
...
// Client owns one paired About socket.
type Client struct{ socket exchange.ClientSocket }

// NewClient constructs an About client from an already validated Exchange
...
func NewClient(configuration exchange.ClientSocketConfiguration) (Client, error) {
	...
	if !sameSocketContract(configuration.Contract, want) {
		return Client{}, transportError(errors.New("about client socket contract differs from About contract"))
	}
...
// Fetch sends one typed About query through Exchange.
...
// Server owns one paired About socket.
type Server struct {
...
func (s Server) Serve(writer http.ResponseWriter, request *http.Request) error {
	if s.service.repository == nil {
		return contractError(errors.New("about server is unconstructed"))
	}
