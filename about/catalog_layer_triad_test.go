...
func TestCatalogSchemaLayerTriad(t *testing.T) {
	...
	t.Run("negative project inventory cannot exceed package and unattributed facts", func(t *testing.T) {
		...
		if !errors.Is(gotErr, core.ErrAboutConflict) {
			t.Fatalf("Catalog.Validate() error = %v, want %v", gotErr, core.ErrAboutConflict)
		}
	})
...
func TestMarkdownReporterLayerTriad(t *testing.T) {
	...
	t.Run("negative invalid record emits no report bytes", func(t *testing.T) {
		...
		if !errors.Is(gotErr, core.ErrAboutContract) || got.Len() != 0 {
			t.Fatalf("WritePackageMarkdown(invalid) = (%d bytes, %v), want (0, errors.Is(..., %v))", got.Len(), gotErr, core.ErrAboutContract)
		}
	})
