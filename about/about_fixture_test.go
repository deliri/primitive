...
func fixtureCatalog(t testing.TB) Catalog {
	...
	path := fixturePath(t, "about")
	featureID := fixtureIdentifier(t, "typed-about")
	...
	usage := Usage{
		ID: fixtureIdentifier(t, "read-about"), Title: fixtureName(t, "Read engineering context"),
	...
	reference := CodeReference{Path: fixturePath(t, "about/catalog.go")}
	assurance := Assurance{
	...
	knowledge := PackageKnowledge{
		...
		Removal: fixtureText(t, "Projects would return to competing and unverifiable About models."),
	...
	packageSnapshot := PackageSnapshot{
		...
		Package:       Package{Key: fixtureIdentifier(t, "about-package"), Subject: subject, Revision: commit, GroupID: groupID, Language: fixtureName(t, "Go"), Knowledge: knowledge},
		Code:          Code{Package: path, Inventory: inventory},
		Evidence:      Evidence{Package: path, Surfaces: []AboutEvidenceSurface{surface}},
	}
	...
	project := Project{
		...
		Capabilities: []ProjectCapability{{FeatureID: featureID, Contributions: []PackageContribution{{Package: path, FeatureID: featureID, Role: fixtureText(t, "Defines and validates the shared About contract."), SurfaceIDs: []Identifier{surfaceID}}}}},
