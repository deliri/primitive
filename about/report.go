...
// about.md projection. The caller owns the destination and its lifecycle.
func WriteProjectMarkdown(destination io.Writer, project Project) error {
	if destination == nil {
		return contractError(errors.New("about project report destination is nil"))
	}
...
// about.md projection, including exact source and evidence references.
func WritePackageMarkdown(destination io.Writer, snapshot PackageSnapshot) error {
	if destination == nil {
		return contractError(errors.New("about package report destination is nil"))
	}
