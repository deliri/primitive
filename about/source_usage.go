...
func (c SourceAnalysisCompleteness) Validate() error {
	return validateEnum(uint8(c), sourceAnalysisLabels(), "about source analysis completeness is invalid")
}
...
func (c SourceAnalysisCompleteness) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(c), sourceAnalysisLabels(), "about source analysis completeness is invalid")
}
func (c *SourceAnalysisCompleteness) UnmarshalJSON(data []byte) error {
	if c == nil {
		return jsonError(errors.New("nil about source analysis receiver"))
	}
	value, err := unmarshalEnum(data, sourceAnalysisLabels(), "about source analysis completeness is invalid")
	if err == nil {
...
func (p FunctionReferencePosture) Validate() error {
	return validateEnum(uint8(p), functionReferenceLabels(), "about function reference posture is invalid")
}
...
func (p FunctionReferencePosture) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(p), functionReferenceLabels(), "about function reference posture is invalid")
}
func (p *FunctionReferencePosture) UnmarshalJSON(data []byte) error {
	if p == nil {
		return jsonError(errors.New("nil about function posture receiver"))
	}
	value, err := unmarshalEnum(data, functionReferenceLabels(), "about function reference posture is invalid")
	if err == nil {
...
func (u FunctionUsage) Validate() error {
	if len(u.ObservedConsumers) > ObservedConsumerMaximum || u.Function.Symbol == nil || u.DeclarationLine == 0 {
		return contractError(errors.New("about function usage shape is invalid"))
	}
	if err := contractJoin(u.Function.Validate(), u.ReferencePosture.Validate()); err != nil {
		return err
	}
	if !functionUsageCountsMatch(u) {
		return conflictError(errors.New("about function usage counts disagree with posture"))
	}
	for index := range u.ObservedConsumers {
		...
		for previous := 0; previous < index; previous++ {
			if u.ObservedConsumers[previous] == u.ObservedConsumers[index] {
				return conflictError(errors.New("about function consumer is duplicated"))
			}
		}
	}
	return nil
...
func (u PackageSourceUsage) Validate() error {
	if len(u.ObservedConsumerPackages) > ObservedConsumerMaximum || len(u.MostReferenced) > FunctionUsageMaximum || len(u.ReviewCandidates) > FunctionUsageMaximum {
		return contractError(errors.New("about package source usage bounds are invalid"))
	}
	...
	if u.DeclarationCount != u.ProductionReferenced+u.RuntimeEntryPoints+u.UnresolvedDeclarations+u.TestReferencedOnly+u.NoReferenceObserved {
		return conflictError(errors.New("about package source usage accounting does not close"))
	}
	if (u.Completeness == SourceAnalysisComplete) != (u.UnresolvedDeclarations == 0) {
		return conflictError(errors.New("about source completeness disagrees with unresolved declarations"))
	}
	...
	for index := range values {
		...
		for previous := 0; previous < index; previous++ {
			if values[previous] == values[index] {
				return conflictError(errors.New("about source path is duplicated"))
			}
		}
	}
	return nil
...
func validateFunctionUsages(owner SourcePath, values []FunctionUsage) error {
	for index := range values {
		...
		if !pathWithin(owner, values[index].Function.Path, true) {
			return conflictError(errors.New("about function usage is outside its package"))
		}
	}
	return nil
}
