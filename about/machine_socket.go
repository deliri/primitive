...
func (q MachineQuery) Validate() error {
	if q.SchemaVersion != SchemaVersion {
		return contractError(errors.New("about machine query schema version is unsupported"))
	}
...
func (q *MachineQuery) UnmarshalJSON(data []byte) error {
	if q == nil {
		return jsonError(errors.New("nil about machine query receiver"))
	}
...
func (r MachineResponse) Validate() error {
	if r.SchemaVersion != SchemaVersion {
		return contractError(errors.New("about machine response schema version is unsupported"))
	}
	...
	if r.Query.Machine != r.Machine.Generation.Configuration.Identity.ID {
		return conflictError(errors.New("about machine response differs from query identity"))
	}
	return nil
}
...
func (r *MachineResponse) UnmarshalJSON(data []byte) error {
	if r == nil {
		return jsonError(errors.New("nil about machine response receiver"))
	}
...
func NewMachineService(repository MachineRepository) (MachineService, error) {
	if repository == nil {
		return MachineService{}, contractError(errors.New("about machine repository is nil"))
	}
...
func (s MachineService) Resolve(ctx context.Context, query MachineQuery) (MachineResponse, error) {
	if s.repository == nil {
		return MachineResponse{}, contractError(errors.New("about machine service is unconstructed"))
	}
...
func NewMachineClient(configuration exchange.ClientSocketConfiguration) (MachineClient, error) {
	...
	if !sameSocketContract(configuration.Contract, want) {
		return MachineClient{}, transportError(errors.New("about machine client socket contract differs from About contract"))
	}
...
func (s MachineServer) Serve(writer http.ResponseWriter, request *http.Request) error {
	if s.service.repository == nil {
		return contractError(errors.New("about machine server is unconstructed"))
	}
