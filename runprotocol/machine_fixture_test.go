...
func TestCurrentMachineDigestBindsTheCompleteValidatedObservation(t *testing.T) {
	t.Parallel()

	current := fixtureCurrentMachine(t)
	first, firstErr := current.Digest()
	second, secondErr := current.Digest()
	if firstErr != nil || secondErr != nil || first != second {
		t.Fatalf("CurrentMachine.Digest() repeated = (%v, %v, %v, %v), want identical nonzero digests and nil errors", first, firstErr, second, secondErr)
	}
	changed := current
	changed.Observation.Runtime.Uptime = fixtureDuration(t, 61)
	changedDigest, changedErr := changed.Digest()
	if changedErr != nil || changedDigest == first {
		t.Fatalf("CurrentMachine.Digest(runtime mutation) = (%v, %v), want a distinct digest and nil error", changedDigest, changedErr)
	}
	invalid := current
	invalid.Generation.ID = MachineGenerationID{}
	invalidDigest, invalidErr := invalid.Digest()
	if !errors.Is(invalidErr, core.ErrRunProtocolContract) || invalidDigest != (core.SHA256Digest{}) {
		t.Fatalf("CurrentMachine.Digest(invalid) = (%v, %v), want zero and errors.Is(..., %v)", invalidDigest, invalidErr, core.ErrRunProtocolContract)
	}
}
