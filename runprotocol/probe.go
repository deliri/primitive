...
func probeRoleLabels() []string { return []string{"", canonicalSelectionText, canonicalExperimentText} }
func (r ProbeRole) Validate() error {
	return validateEnum(uint8(r), probeRoleLabels(), probeRoleInvalidDiagnostic)
}
...
func (r ProbeRole) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(r), probeRoleLabels(), probeRoleInvalidDiagnostic)
}
func (r *ProbeRole) UnmarshalJSON(data []byte) error {
	if r == nil {
		return jsonError(errors.New("nil run protocol probe role receiver"))
	}
	value, err := unmarshalEnum(data, probeRoleLabels(), probeRoleInvalidDiagnostic)
	if err == nil {
		*r = ProbeRole(value)
	}
	return err
...
const (
	...
	ProbeKindExternalSuite
	ProbeKindTool
	probeKindLimit
)

func probeKindLabels() []string {
	return []string{"", "go_file_selection", "go_package_selection", "ci_selection", "go_test", "go_race", "go_benchmark", "go_fuzz", "go_diagnostic_profile", "javascript_test", externalSuiteText, canonicalToolText}
}
func (k ProbeKind) Validate() error {
	return validateEnum(uint8(k), probeKindLabels(), probeKindInvalidDiagnostic)
}
...
func (k ProbeKind) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(k), probeKindLabels(), probeKindInvalidDiagnostic)
}
func (k *ProbeKind) UnmarshalJSON(data []byte) error {
	if k == nil {
		return jsonError(errors.New("nil run protocol probe kind receiver"))
	}
	value, err := unmarshalEnum(data, probeKindLabels(), probeKindInvalidDiagnostic)
	if err == nil {
		*k = ProbeKind(value)
	}
	return err
...
func probeTargetLabels() []string {
	return []string{"", "go_declaration", "go_file", "go_package", "javascript_file", externalSuiteText, canonicalToolText, "ci_plan"}
}
func (k ProbeTargetKind) Validate() error {
	return validateEnum(uint8(k), probeTargetLabels(), probeTargetKindInvalidDiagnostic)
}
...
func (k ProbeTargetKind) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(k), probeTargetLabels(), probeTargetKindInvalidDiagnostic)
}
func (k *ProbeTargetKind) UnmarshalJSON(data []byte) error {
	if k == nil {
		return jsonError(errors.New("nil run protocol probe target receiver"))
	}
	value, err := unmarshalEnum(data, probeTargetLabels(), probeTargetKindInvalidDiagnostic)
	if err == nil {
...
func (t GoFileTarget) Validate() error {
	...
	if !strings.HasSuffix(t.File.String(), GoTestFileSuffix) {
		return contractError(errors.New("run protocol Go file selection requires a _test.go source file"))
	}
	return validateGoSelectionChildKinds(t.ChildKinds, "run protocol Go file child kinds are invalid")
}
...
func (t GoPackageTarget) Validate() error {
	...
	return validateGoSelectionChildKinds(t.ChildKinds, "run protocol Go package child kinds are invalid")
}
...
func (t ProbeTarget) Validate() error {
	...
	if t.payloadCount() != 1 {
		return contractError(errors.New("run protocol probe target must carry exactly one variant"))
	}
...
func (t ProbeTarget) validateVariant() error {
	...
	case ProbeTargetExternalSuite:
		return validateTargetVariant(t.ExternalSuite)
	...
	case ProbeTargetUnknown:
		return contractError(errors.New(probeTargetEscapedDiagnostic))
	default:
		return contractError(errors.New(probeTargetEscapedDiagnostic))
	}
}
...
func validateTargetVariant[T interface{ Validate() error }](value *T) error {
	if value == nil {
		return conflictError(errors.New("run protocol probe target variant differs from kind"))
	}
...
func (t ProbeTarget) payloadCount() int {
	count := 0
	for _, present := range [...]bool{t.GoDeclaration != nil, t.GoFile != nil, t.GoPackage != nil, t.JavaScript != nil, t.ExternalSuite != nil, t.Tool != nil, t.CI != nil} {
		if present {
			count++
		}
	}
...
func (p SelectionParent) Validate() error {
	...
	if role != ProbeRoleSelection || !targetAdmitsKind(p.Target.Kind, p.Kind) {
		return conflictError(errors.New("run protocol selection parent kind and target disagree"))
	}
	return nil
}
...
func (r RequestedProbe) Validate() error {
	...
	if r.Subject.Repository.value != r.Source.Repository.value {
		return conflictError(errors.New("run protocol requested probe source repository differs from subject"))
	}
...
func validateRequestedKinds(target ProbeTarget, kinds []ProbeKind) error {
	if len(kinds) == 0 || len(kinds) > ProbeKindMaximum {
		return contractError(errors.New("run protocol requested probe kinds are invalid"))
	}
	...
	if !requestedKindsMatchSelectionTarget(target, kinds) {
		return conflictError(errors.New("run protocol requested probe kinds differ from selection child kinds"))
	}
	return nil
}
...
func validateRequestedKindAt(target ProbeTarget, kinds []ProbeKind, index int) error {
	...
	if duplicateProbeKind(kinds, index) {
		return conflictError(errors.New("run protocol requested probe kind is duplicated"))
	}
	if index > 0 && kinds[index-1] >= kinds[index] {
		return conflictError(errors.New("run protocol requested probe kinds are not canonical"))
	}
	if !requestedTargetAdmitsKind(target.Kind, kinds[index]) {
		return conflictError(errors.New("run protocol requested probe kind differs from target"))
	}
	return nil
}
...
func AdmitRequestedProbe(request RequestedProbe, environment AdmittedEnvironment) (ProbeIdentity, error) {
	...
	if !environment.Satisfies(request.Constraints) {
		return ProbeIdentity{}, conflictError(errors.New("run protocol admitted environment does not satisfy the requested requirement"))
	}
...
func (i ProbeIdentity) Validate() error {
	...
	if i.Subject.Repository != i.Source.Repository {
		return conflictError(errors.New("run protocol probe identity source repository differs from subject"))
	}
...
func (i ProbeIdentity) validateRelationship() error {
	...
	if role != i.Role || !targetAdmitsKind(i.Target.Kind, i.Kind) {
		return conflictError(errors.New("run protocol probe role, kind, and target disagree"))
	}
	if i.Role == ProbeRoleSelection && i.Parent != nil {
		return conflictError(errors.New("run protocol selection probe cannot descend from another selection"))
	}
	if i.Parent != nil {
...
func (i ProbeIdentity) validateParent() error {
	...
	if !experimentDescendsFromSelection(i, *i.Parent) {
		return conflictError(errors.New("run protocol experiment does not descend from its selection parent"))
	}
	return nil
}
...
func validateGoSelectionChildKinds(values []ProbeKind, diagnostic string) error {
	...
	for index := range values {
		if !goDeclarationExperiment(values[index]) {
			return conflictError(errors.New("run protocol Go selection child kind is not a Go experiment"))
		}
		if duplicateProbeKind(values, index) || (index > 0 && values[index-1] >= values[index]) {
			return conflictError(errors.New("run protocol Go selection child kinds are not canonical"))
		}
	}
	return nil
}
...
func targetAdmitsKind(target ProbeTargetKind, kind ProbeKind) bool {
	...
	if target == ProbeTargetExternalSuite {
		return kind == ProbeKindExternalSuite
	}
