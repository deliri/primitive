...
func TestDeriveRequestIdentityBindsCallerNonceToOrigin(t *testing.T) {
	...
	if got, gotErr := DeriveRequestIdentity(OriginIdentity{}, nonce); got != (RequestIdentity{}) || !errors.Is(gotErr, core.ErrRunProtocolContract) {
		t.Fatalf("DeriveRequestIdentity(zero origin) = (%v, %v), want zero and errors.Is(..., %v)", got, gotErr, core.ErrRunProtocolContract)
	}
	if got, gotErr := DeriveRequestIdentity(origin, RequestNonce{}); got != (RequestIdentity{}) || !errors.Is(gotErr, core.ErrRunProtocolContract) {
		t.Fatalf("DeriveRequestIdentity(zero nonce) = (%v, %v), want zero and errors.Is(..., %v)", got, gotErr, core.ErrRunProtocolContract)
	}
}
