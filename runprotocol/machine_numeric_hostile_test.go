	...
	t.Run("IOPS retains operation units", func(t *testing.T) {
		...
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.wantErr && !errors.Is(gotErr, core.ErrRunProtocolContract) {
					t.Fatalf("NewMachineIOPS(%d) error = %v, want errors.Is(..., %v)", tc.value, gotErr, core.ErrRunProtocolContract)
				}
			})
		}
	})
	...
	t.Run("byte throughput retains byte units", func(t *testing.T) {
		...
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.wantErr && !errors.Is(gotErr, core.ErrRunProtocolContract) {
					t.Fatalf("NewMachineBytesPerSecond(%d) error = %v, want errors.Is(..., %v)", tc.value, gotErr, core.ErrRunProtocolContract)
				}
			})
		}
	})
	...
	t.Run("bit throughput retains bit units", func(t *testing.T) {
		...
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.wantErr && !errors.Is(gotErr, core.ErrRunProtocolContract) {
					t.Fatalf("NewMachineBitsPerSecond(%d) error = %v, want errors.Is(..., %v)", tc.value, gotErr, core.ErrRunProtocolContract)
				}
			})
		}
	})
