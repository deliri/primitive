...
func TestMachineObservationExecutionSettingsBindsObservedCapacity(t *testing.T) {
	...
	t.Run("invalid observation cannot issue execution settings", func(t *testing.T) {
		...
		got, gotErr := observation.ExecutionSettings()
		if !errors.Is(gotErr, core.ErrRunProtocolContract) || got != (MachineExecutionSettings{}) {
			t.Fatalf("MachineObservation.ExecutionSettings(zero CPUs) = (%+v, %v), want zero and errors.Is(..., %v)", got, gotErr, core.ErrRunProtocolContract)
		}
	})

	t.Run("detached zero execution settings fail closed", func(t *testing.T) {
		t.Parallel()
		if gotErr := (MachineExecutionSettings{}).Validate(); !errors.Is(gotErr, core.ErrRunProtocolContract) {
			t.Fatalf("MachineExecutionSettings{}.Validate() error = %v, want errors.Is(..., %v)", gotErr, core.ErrRunProtocolContract)
		}
	})
}
