...
func dispositionLabels() []string { return []string{"", "disposition_admitted", "disposition_refused"} }
func (k DispositionKind) Validate() error {
	return validateEnum(uint8(k), dispositionLabels(), dispositionKindInvalidDiagnostic)
}
...
func (k DispositionKind) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(k), dispositionLabels(), dispositionKindInvalidDiagnostic)
}
func (k *DispositionKind) UnmarshalJSON(data []byte) error {
	if k == nil {
		return jsonError(errors.New("nil run protocol disposition receiver"))
	}
	value, err := unmarshalEnum(data, dispositionLabels(), dispositionKindInvalidDiagnostic)
	if err == nil {
...
func refusalLabels() []string {
	return []string{"", "unauthorized", "repository", "probe", "profile", "budget", canonicalSourceText, "conflict"}
}
func (r RefusalReason) Validate() error {
	return validateEnum(uint8(r), refusalLabels(), refusalReasonInvalidDiagnostic)
}
...
func (r RefusalReason) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(r), refusalLabels(), refusalReasonInvalidDiagnostic)
}
func (r *RefusalReason) UnmarshalJSON(data []byte) error {
	if r == nil {
		return jsonError(errors.New("nil run protocol refusal reason receiver"))
	}
	value, err := unmarshalEnum(data, refusalLabels(), refusalReasonInvalidDiagnostic)
	if err == nil {
...
func terminalLabels() []string {
	return []string{"", "terminal_completed", "terminal_failed", "terminal_" + canonicalCancelledText, "terminal_" + canonicalTimedOutText, "terminal_unavailable", "terminal_not_run"}
}
func (s TerminalState) Validate() error {
	return validateEnum(uint8(s), terminalLabels(), terminalStateInvalidDiagnostic)
}
...
func (s TerminalState) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(s), terminalLabels(), terminalStateInvalidDiagnostic)
}
func (s *TerminalState) UnmarshalJSON(data []byte) error {
	if s == nil {
		return jsonError(errors.New("nil run protocol terminal state receiver"))
	}
	value, err := unmarshalEnum(data, terminalLabels(), terminalStateInvalidDiagnostic)
	if err == nil {
...
func observationLabels() []string {
	return []string{"", canonicalSelectionText, canonicalExperimentText, "infrastructure"}
}
func (k ObservationKind) Validate() error {
	return validateEnum(uint8(k), observationLabels(), observationKindInvalidDiagnostic)
}
...
func (k ObservationKind) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(k), observationLabels(), observationKindInvalidDiagnostic)
}
func (k *ObservationKind) UnmarshalJSON(data []byte) error {
	if k == nil {
		return jsonError(errors.New("nil run protocol observation kind receiver"))
	}
	value, err := unmarshalEnum(data, observationLabels(), observationKindInvalidDiagnostic)
	if err == nil {
...
func infrastructureStageLabels() []string {
	return []string{"", "infrastructure_admission", "infrastructure_boot", "infrastructure_readiness", "infrastructure_" + canonicalSourceText, "infrastructure_expansion", "infrastructure_execution", "infrastructure_delivery", "infrastructure_cleanup"}
}
func (s InfrastructureStage) Validate() error {
	return validateEnum(uint8(s), infrastructureStageLabels(), infrastructureStageInvalidDiagnostic)
}
...
func (s InfrastructureStage) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(s), infrastructureStageLabels(), infrastructureStageInvalidDiagnostic)
}
func (s *InfrastructureStage) UnmarshalJSON(data []byte) error {
	if s == nil {
		return jsonError(errors.New("nil run protocol infrastructure stage receiver"))
	}
	value, err := unmarshalEnum(data, infrastructureStageLabels(), infrastructureStageInvalidDiagnostic)
	if err == nil {
...
func (s EvidenceSurface) Validate() error {
	if err := contractJoin(s.ID.Validate(), s.Subject.Validate(), s.Target.Validate()); err != nil {
		return err
	}
	if len(s.EligibleKinds) == 0 || len(s.EligibleKinds) > ProbeKindMaximum || len(s.Profiles) == 0 || len(s.Profiles) > ProbeKindMaximum {
		return contractError(errors.New("run protocol evidence surface bounds are invalid"))
	}
...
func validateSurfaceKinds(target ProbeTarget, kinds []ProbeKind) error {
	for index := range kinds {
		...
		if !requestedTargetAdmitsKind(target.Kind, kinds[index]) || duplicateProbeKind(kinds, index) {
			return conflictError(errors.New("run protocol evidence surface kind is contradictory"))
		}
		if index > 0 && kinds[index-1] >= kinds[index] {
			return conflictError(errors.New("run protocol evidence surface kinds are not canonical"))
		}
	}
	if !requestedKindsMatchSelectionTarget(target, kinds) {
		return conflictError(errors.New("run protocol evidence surface kinds differ from selection child kinds"))
	}
	return nil
}
	...
	for index := range values {
		...
		for previous := range index {
			if values[previous] == values[index] {
				return conflictError(errors.New("run protocol profile identity is duplicated"))
			}
		}
	}
	return nil
...
func (d RequestDisposition) Validate() error {
	...
	if (d.Admitted != nil) == (d.Refused != nil) {
		return contractError(errors.New("run protocol disposition must carry exactly one variant"))
	}
	...
	return conflictError(errors.New("run protocol disposition payload differs from kind"))
}
...
func (r RequestReference) Validate() error {
...
func (r RequestReference) ValidateFor(surface EvidenceSurface) error {
	...
	if r.SurfaceID.value != surface.ID.value || !sameSubject(r.Requested.Subject, surface.Subject) || !sameTarget(r.Requested.Target, surface.Target) || r.Source != r.Requested.Source {
		return conflictError(errors.New("run protocol request does not descend from its evidence surface"))
	}
	if !requestedKindsAllowed(r.Requested.Kinds, surface.EligibleKinds) || !profileAllowed(r.Requested.Profile, surface.Profiles) {
		return conflictError(errors.New("run protocol request exceeds its evidence surface"))
	}
...
func validateAdmissionForRequest(admission Admission, requested RequestedProbe) error {
	if !sameSubject(admission.Probe.Subject, requested.Subject) || admission.Probe.Origin != requested.Origin || admission.Probe.Source != requested.Source || !sameTarget(admission.Probe.Target, requested.Target) || admission.Probe.Profile != requested.Profile || !admission.Probe.Environment.Satisfies(requested.Constraints) || admission.Probe.Parent != nil {
		return conflictError(errors.New("run protocol admission differs from requested probe"))
	}
	...
	if admission.Probe.Kind != want {
		return conflictError(errors.New("run protocol admitted probe kind differs from requested target"))
	}
	return nil
}
...
func admittedKindForRequest(requested RequestedProbe) (ProbeKind, error) {
	if len(requested.Kinds) == 0 {
		return ProbeKindUnknown, contractError(errors.New("run protocol requested probe has no kind"))
	}
	...
	case ProbeTargetGoDeclaration:
		return soleRequestedKind(requested, "run protocol direct declaration admission requires one requested kind")
	case ProbeTargetJavaScriptFile:
		return ProbeKindJavaScriptTest, nil
	case ProbeTargetExternalSuite:
		return ProbeKindExternalSuite, nil
	case ProbeTargetTool:
		return ProbeKindTool, nil
	case ProbeTargetUnknown:
		return ProbeKindUnknown, contractError(errors.New(admittedTargetInvalidDiagnostic))
	default:
		return ProbeKindUnknown, contractError(errors.New(admittedTargetInvalidDiagnostic))
	}
}
...
func (m DecimalMeasurement) Validate() error {
	if m.Coefficient == 0 || m.Scale > DecimalMeasurementScaleMaximum {
		return contractError(errors.New("run protocol decimal measurement is invalid"))
	}
	if m.Scale > 0 && m.Coefficient%10 == 0 {
		return contractError(errors.New("run protocol decimal measurement is not canonical"))
	}
	return nil
}
...
func (m BenchmarkMeasurement) Validate() error {
	...
	if m.Iterations == 0 {
		return contractError(errors.New("run protocol benchmark measurement is invalid"))
	}
...
func cachePostureLabels() []string {
	return []string{"", "cache_disabled", "cache_eligible", "cache_used"}
}
func (p CachePosture) Validate() error {
	return validateEnum(uint8(p), cachePostureLabels(), cachePostureInvalidDiagnostic)
}
...
func (p CachePosture) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(p), cachePostureLabels(), cachePostureInvalidDiagnostic)
}
func (p *CachePosture) UnmarshalJSON(data []byte) error {
	if p == nil {
		return jsonError(errors.New("nil run protocol cache posture receiver"))
	}
	value, err := unmarshalEnum(data, cachePostureLabels(), cachePostureInvalidDiagnostic)
	if err == nil {
...
func (a ExecutionAttempt) Validate() error {
	...
	if a.Sequence == 0 || a.Planned == 0 {
		return contractError(errors.New("run protocol execution attempt sequence or planned denominator is zero"))
	}
	...
	if total != uint64(a.Planned) {
		return conflictError(errors.New("run protocol execution attempt outcome counts do not close against its planned denominator"))
	}
	return nil
}
...
func (a ExecutionAccounting) Validate() error {
	if len(a.Attempts) == 0 || len(a.Attempts) > ExecutionAttemptMaximum {
		return contractError(errors.New("run protocol execution attempt sequence is empty or exceeds its bound"))
	}
	for index := range a.Attempts {
		...
		if a.Attempts[index].Sequence != wantSequence {
			return conflictError(errors.New("run protocol execution attempts are not in canonical append-only sequence"))
		}
		if index > 0 && (a.Attempts[index].Planned != a.Attempts[0].Planned || a.Attempts[index].Filtered != a.Attempts[0].Filtered) {
			return conflictError(errors.New("run protocol execution retry changed the planned denominator or filter scope"))
		}
	}
	return nil
}
...
type ExperimentMeasurements struct {
	...
	Scaling             []ScalingCapture       `json:"scaling"`
...
func (m ExperimentMeasurements) Validate() error {
	if m.CoverageBasisPoints != nil && *m.CoverageBasisPoints > 10_000 {
		return contractError(errors.New("run protocol coverage exceeds one hundred percent"))
	}
	if len(m.Benchmarks) > BenchmarkMeasurementMaximum || len(m.Scaling) > ScalingCaptureMaximum {
		return contractError(errors.New("run protocol measurement collection exceeds its bound"))
	}
	if m.Accounting != nil {
	...
	return validateMeasurementCollections(m.Benchmarks, m.Scaling)
}

func validateMeasurementCollections(benchmarks []BenchmarkMeasurement, scaling []ScalingCapture) error {
	for index := range benchmarks {
		if err := benchmarks[index].Validate(); err != nil {
			return err
		}
	}
	for index := range scaling {
		if err := scaling[index].Validate(); err != nil {
			return err
		}
	}
	return nil
...
func (o SelectionObservation) Validate() error {
	...
	if o.Admitted+o.Refused != o.Planned || o.Executed+o.NotRun != o.Admitted {
		return conflictError(errors.New("run protocol selection accounting does not close"))
	}
	return nil
}
...
func (o ExperimentObservation) Validate() error {
	if len(o.Artifacts) > ArtifactReferenceMaximum {
		return contractError(errors.New("run protocol experiment artifact count exceeds its bound"))
	}
...
func (o ExperimentObservation) validateExecutionState() error {
	if o.Started {
		if o.Measurements.DurationNs == 0 || o.Outcome == OutcomeNotRun {
			return conflictError(errors.New("run protocol started experiment has contradictory execution facts"))
		}
		return nil
	}
	if !o.Measurements.isZero() || len(o.Artifacts) != 0 || !outcomePermitsUnstarted(o.Outcome) {
		return conflictError(errors.New("run protocol unstarted experiment has contradictory execution facts"))
	}
	return nil
}

func (m ExperimentMeasurements) isZero() bool {
	return m.DurationNs == 0 && m.PeakMemoryBytes == 0 && m.CoverageBasisPoints == nil && len(m.Benchmarks) == 0 && len(m.Scaling) == 0 && m.Accounting == nil
}
...
func (o InfrastructureObservation) Validate() error {
	if len(o.PartialEvidence) > ArtifactReferenceMaximum {
		return contractError(errors.New("run protocol partial evidence exceeds its bound"))
	}
	...
	for index := range values {
		...
		for previous := range index {
			if values[previous].Path.value == values[index].Path.value {
				return conflictError(errors.New("run protocol artifact path is duplicated"))
			}
		}
	}
	return nil
...
func (r ObservationReference) Validate() error {
	...
	if !r.independentAuthorities() {
		return conflictError(errors.New("run protocol evidence authorities are not independent of requester and subject"))
	}
	if r.payloadCount() != 1 {
		return contractError(errors.New("run protocol observation must carry exactly one variant"))
	}
	return r.validatePayload()
}

func (r ObservationReference) independentAuthorities() bool {
...
func (r ObservationReference) validatePayload() error {
	switch r.Kind {
	...
	case ObservationUnknown:
		return conflictError(errors.New(observationPayloadDisagreementDiagnostic))
	default:
		return conflictError(errors.New(observationPayloadDisagreementDiagnostic))
	}
}

func (r ObservationReference) validateSelectionPayload() error {
	return validateObservationPayload(r.Selection, r.Probe.Role == ProbeRoleSelection && r.Terminal == TerminalCompleted)
}

func (r ObservationReference) validateExperimentPayload() error {
	...
	if !experimentTerminalMatches(r.Experiment.Outcome, r.Terminal) {
		return conflictError(errors.New("run protocol experiment outcome and terminal state disagree"))
	}
	return nil
}

func (r ObservationReference) validateInfrastructurePayload() error {
	return validateObservationPayload(r.Infrastructure, r.Terminal != TerminalCompleted)
}

func (r ObservationReference) validateExperimentBinding() error {
	if r.Experiment.EnvironmentFingerprint != r.Probe.Environment.EnvironmentFingerprint || r.Experiment.MachineSheetDigest != r.Probe.Environment.MachineSheetDigest {
		return conflictError(errors.New("run protocol experiment environment differs from admitted probe"))
	}
	for _, capture := range r.Experiment.Measurements.Scaling {
		if capture.EnvironmentFingerprint != r.Experiment.EnvironmentFingerprint || capture.Profile != r.Probe.Profile {
			return conflictError(errors.New("run protocol scaling capture differs from experiment identity"))
		}
	}
	return nil
}
...
func TerminalForOutcome(outcome Outcome) (TerminalState, error) {
	...
	return TerminalUnknown, contractError(errors.New("run protocol experiment outcome has no terminal state"))
}
...
func (r ObservationReference) payloadCount() int {
	count := 0
...
func (r ObservationReference) ValidateFor(request RequestReference) error {
	if err := contractJoin(r.Validate(), request.Validate()); err != nil {
		return err
	}
	if request.Disposition.Admitted == nil {
		return conflictError(errors.New("run protocol observation descends from a refused request"))
	}
	admission := request.Disposition.Admitted
	if !r.matchesAdmission(request, *admission) {
		return conflictError(errors.New("run protocol observation does not descend from its admission"))
	}
	if r.Probe.Source != r.Source {
		return conflictError(errors.New("run protocol observation probe source differs from observation source"))
	}
	if r.Probe.Parent != nil && !r.matchesParent(*admission) {
		return conflictError(errors.New("run protocol observation child differs from admitted selection"))
	}
	return nil
}

func (r ObservationReference) matchesAdmission(request RequestReference, admission Admission) bool {
...
func (r ObservationReference) matchesParent(admission Admission) bool {
...
func sameTarget(left, right ProbeTarget) bool {
	...
	if left.Kind == ProbeTargetExternalSuite {
		return sameComparableTarget(left.ExternalSuite, right.ExternalSuite)
	}
