...
func TestExperimentOutcomeTerminalRelationshipExhaustive(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "unknown outcome never becomes a terminal fact", outcome: OutcomeUnknown, terminal: TerminalCompleted, wantErr: core.ErrRunProtocolContract},
		...
		{name: "one fact terminal mutation cannot contradict passed outcome", outcome: OutcomePassed, terminal: TerminalFailed, wantErr: core.ErrRunProtocolConflict},
	}
	...
	for _, current := range cases {
		t.Run(current.name, func(t *testing.T) {
			...
			if current.outcome == OutcomeUnknown {
				if mapped != TerminalUnknown || !errors.Is(mappingErr, core.ErrRunProtocolContract) {
					t.Fatalf("TerminalForOutcome(%v) = (%v, %v), want (%v, errors.Is(..., %v))",
						current.outcome, mapped, mappingErr, TerminalUnknown, core.ErrRunProtocolContract)
				}
			...
			if current.wantErr == nil && gotErr != nil {
				t.Fatalf("ObservationReference.Validate() error = %v, want nil", gotErr)
			}
			if current.wantErr != nil && !errors.Is(gotErr, current.wantErr) {
				t.Fatalf("ObservationReference.Validate() error = %v, want %v", gotErr, current.wantErr)
			}
		})
	}
}
...
func TestObservationAdmissionBindingRejectsOneFactTargetMutation(t *testing.T) {
	...
	if gotErr := observation.ValidateFor(request); gotErr != nil {
		t.Fatalf("ObservationReference.ValidateFor(exact admission) error = %v, want nil", gotErr)
	}
	mutated := observation
	...
	if !errors.Is(gotErr, core.ErrRunProtocolConflict) {
		t.Fatalf("ObservationReference.ValidateFor(mutated target) error = %v, want %v", gotErr, core.ErrRunProtocolConflict)
	}
}
...
func TestRequestAdmissionBindingRejectsOneFactEnvironmentMutation(t *testing.T) {
	...
	if gotErr := baseline.ValidateFor(surface); gotErr != nil {
		t.Fatalf("RequestReference.ValidateFor(exact environment) error = %v, want nil", gotErr)
	}
	mutated := baseline
	...
	if !errors.Is(gotErr, core.ErrRunProtocolConflict) {
		t.Fatalf("RequestReference.ValidateFor(mutated admitted environment) error = %v, want %v", gotErr, core.ErrRunProtocolConflict)
	}
}
...
func TestObservationIndependentAuthorityAndEnvironmentBindings(t *testing.T) {
	...
	if gotErr := baseline.Validate(); gotErr != nil {
		t.Fatalf("ObservationReference.Validate(exact authorities and environment) error = %v, want nil", gotErr)
	}
	...
	t.Run("subject cannot verify its own observation", func(t *testing.T) {
		...
		if !errors.Is(gotErr, core.ErrRunProtocolConflict) {
			t.Fatalf("ObservationReference.Validate(self verifier) error = %v, want %v", gotErr, core.ErrRunProtocolConflict)
		}
	})
	...
	t.Run("experiment environment cannot differ from admitted probe", func(t *testing.T) {
		...
		gotErr := mutated.Validate()
		if !errors.Is(gotErr, core.ErrRunProtocolConflict) {
			t.Fatalf("ObservationReference.Validate(mutated environment) error = %v, want %v", gotErr, core.ErrRunProtocolConflict)
		}
	})
}

func fixtureExperimentObservationReference(t testing.TB, outcome Outcome, terminal TerminalState) ObservationReference {
	t.Helper()
...
func fixtureAdmittedObservation(t testing.TB) (RequestReference, ObservationReference) {
	t.Helper()
	uuid := fixtureRunProtocolUUID(t)
	...
	target := ProbeTarget{
		...
		GoDeclaration: &GoDeclarationTarget{
			Module: fixtureIdentifier(t, "primitive"), Package: fixturePath(t, "runprotocol"),
			File: fixturePath(t, "runprotocol/evidence.go"), Symbol: fixtureName(t, "TestEvidence"),
		},
	}
