...
func sourceImportKindLabels() []string {
	return []string{"", "standard_library", canonicalProjectText, "external"}
}

func (k SourceImportKind) Validate() error {
	return validateEnum(uint8(k), sourceImportKindLabels(), sourceImportKindInvalidDiagnostic)
}
...
func (k SourceImportKind) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(k), sourceImportKindLabels(), sourceImportKindInvalidDiagnostic)
}
func (k *SourceImportKind) UnmarshalJSON(data []byte) error {
	if k == nil {
		return jsonError(errors.New("nil standard source import kind receiver"))
	}
	value, err := unmarshalEnum(data, sourceImportKindLabels(), sourceImportKindInvalidDiagnostic)
	if err == nil {
...
func (i SourceImport) Validate() error {
	...
	if i.Kind == SourceImportKindProject {
		if i.ProjectModule == nil {
			return conflictError(errors.New("standard project import omits its module identity"))
		}
		return i.validateProjectPath()
	}
	if i.ProjectModule != nil {
		return conflictError(errors.New("standard non-project import carries a module identity"))
	}
	return nil
}
...
func (i SourceImport) validateProjectPath() error {
	...
	if importPath != module && !strings.HasPrefix(importPath, module+"/") {
		return conflictError(errors.New("standard project import is outside its module"))
	}
	return nil
}
...
func (i SourceFileImports) Validate() error {
	if len(i.Values) > SourceFileImportMaximum {
		return contractError(errors.New("standard source import count exceeds its bound"))
	}
	for index := range i.Values {
		...
		if index > 0 && i.Values[index-1].Path.String() >= i.Values[index].Path.String() {
			return conflictError(errors.New("standard source imports are duplicated or not in canonical order"))
		}
	}
	return nil
}
