...
func (c Code) Validate() error {
	if len(c.Components) > PackageComponentMaximum {
		return contractError(errors.New("standard code component count exceeds its bound"))
	}
...
func (c Code) validateFiles() error {
	...
	if c.Files.Package != c.Package {
		return conflictError(errors.New("standard file catalog package differs from code package"))
	}
	if !summarizePackageFiles(c.Files.Files).matches(c.Inventory) {
		return conflictError(errors.New("standard file catalog does not close against inventory"))
	}
	return nil
}
...
func (c Code) validateComponents() error {
	for index := range c.Components {
		...
		if c.Components[index].Package != c.Package {
			return conflictError(errors.New("standard component package differs from code package"))
		}
		for previous := range index {
			if c.Components[previous].Path == c.Components[index].Path {
				return conflictError(errors.New("standard component path is duplicated"))
			}
		}
	}
	return nil
...
func (c Code) validateSourceUsage() error {
	...
	if c.SourceUsage.Package != c.Package {
		return conflictError(errors.New("standard source usage package differs from code package"))
	}
	return nil
}
...
type Evidence struct {
	Package      SourcePath             `json:"package"`
	Surfaces     []EvidenceSurface      `json:"surfaces"`
	Requests     []RequestReference     `json:"requests"`
	Observations []ObservationReference `json:"observations"`
}
...
func (e Evidence) validateShape() error {
	...
	if len(e.Surfaces) == 0 || len(e.Surfaces) > EvidenceSurfaceMaximum || len(e.Requests) > RequestReferenceMaximum || len(e.Observations) > ObservationReferenceMaximum {
		return contractError(errors.New("standard evidence shape is invalid"))
	}
	return nil
}
...
func (s EvidenceSummary) Validate() error {
	if s.SurfaceCount > EvidenceSurfaceMaximum || s.RequestedCount > RequestReferenceMaximum || s.ObservedCount > ObservationReferenceMaximum {
		return contractError(errors.New("standard evidence summary exceeds its bounds"))
	}
	if s.AdmittedCount+s.RefusedCount != s.RequestedCount || s.CurrentCount+s.StaleCount != s.ObservedCount {
		return conflictError(errors.New("standard evidence summary accounting does not close"))
	}
	if s.SelectionCount+s.PassedCount+s.FailedCount+s.NonAcceptingCount != s.ObservedCount {
		return conflictError(errors.New("standard evidence outcome accounting does not close"))
	}
	return nil
}
...
func (s SourceUsageSummary) Validate() error {
	...
	if s.DeclarationCount != s.ProductionReferenced+s.RuntimeEntryPoints+s.UnresolvedDeclarations+s.TestReferencedOnly+s.NoReferenceObserved {
		return conflictError(errors.New("standard source usage summary accounting does not close"))
	}
	if s.ObservedConsumerPackages > ObservedConsumerMaximum {
		return contractError(errors.New("standard source usage consumer count exceeds its bound"))
	}
	return nil
}
...
func (c PackageContribution) Validate() error {
	if len(c.SurfaceIDs) > EvidenceSurfaceMaximum {
		return contractError(errors.New("standard contribution surface count exceeds its bound"))
	}
...
func (c ProjectCapability) Validate() error {
	if len(c.Contributions) == 0 || len(c.Contributions) > PackageContributionMaximum {
		return contractError(errors.New("standard capability contribution count is invalid"))
	}
	...
	for index := range c.Contributions {
		...
		for previous := range index {
			...
			if left.Package == right.Package && left.FeatureID == right.FeatureID {
				return conflictError(errors.New("standard capability contribution is duplicated"))
			}
		}
	}
	return nil
...
// PackageSnapshot is one bounded package Standard record at a selected revision.
...
func (s PackageSnapshot) Validate() error {
	if s.SchemaVersion != SchemaVersion {
		return contractError(errors.New("standard package snapshot schema version is unsupported"))
	}
	...
	if s.Code.Package != s.Package.Knowledge.Path || s.Evidence.Package != s.Package.Knowledge.Path {
		return conflictError(errors.New("standard package, code, and evidence paths disagree"))
	}
	if s.Code.SourceUsage != nil && s.Code.SourceUsage.Revision != s.Package.Revision {
		return conflictError(errors.New("standard source usage revision differs from package revision"))
	}
	...
	for index := range e.Surfaces {
		...
		for previous := range index {
			if e.Surfaces[previous].ID == e.Surfaces[index].ID {
				return conflictError(errors.New("standard evidence surface identity is duplicated"))
			}
		}
	}
	return nil
...
func (s PackageSnapshot) validateAuthoredSurfaceReferences() error {
	for _, surface := range s.Evidence.Surfaces {
		if !sameSubject(surface.Subject, s.Package.Subject) {
			return conflictError(errors.New("standard evidence surface subject differs from package subject"))
		}
	}
	for _, control := range [...]AssuranceControl{s.Package.Knowledge.AuthorAssurance.Policy, s.Package.Knowledge.AuthorAssurance.Validation, s.Package.Knowledge.AuthorAssurance.Effects, s.Package.Knowledge.AuthorAssurance.Proof} {
		for _, id := range control.SurfaceIDs {
			if !surfaceExists(s.Evidence.Surfaces, id) {
				return conflictError(errors.New("standard assurance names an absent evidence surface"))
			}
		}
	}
	for _, claim := range s.Package.Knowledge.AuthorComplexity {
		if !surfaceExists(s.Evidence.Surfaces, claim.SurfaceID) {
			return conflictError(errors.New("standard complexity claim names an absent evidence surface"))
		}
	}
	return nil
}
...
func (e Evidence) validateRequests() error {
	for index := range e.Requests {
		...
		if !ok {
			return conflictError(errors.New("standard request names an absent evidence surface"))
		}
		...
		for previous := range index {
			if e.Requests[previous].Request == e.Requests[index].Request {
				return conflictError(errors.New("standard request identity is duplicated"))
			}
		}
	}
	return nil
...
func (e Evidence) validateObservations() error {
	for index := range e.Observations {
		...
		if !ok {
			return conflictError(errors.New("standard observation names an absent request"))
		}
		...
		for previous := range index {
			if e.Observations[previous].Observation == e.Observations[index].Observation || e.Observations[previous].EnvelopeDigest == e.Observations[index].EnvelopeDigest {
				return conflictError(errors.New("standard observation identity or envelope is duplicated"))
			}
		}
	}
	return nil
...
func validateSelectionExpansion(values []ObservationReference, child ObservationReference) error {
	...
	for index := range values {
		...
		if candidate.Selection.ExpansionIdentity != child.Probe.Parent.ExpansionDigest {
			return conflictError(errors.New("standard child experiment expansion differs from selection observation"))
		}
		return nil
	}
	return conflictError(errors.New("standard child experiment has no selection observation"))
}
...
func (s PackageSummary) Validate() error {
	if s.FeatureCount > FeatureMaximum || s.ComponentCount > PackageComponentMaximum || s.ComplexityClaimCount > ComplexityClaimMaximum {
		return contractError(errors.New("standard package summary count exceeds its bound"))
	}
...
func (s *EvidenceSummary) addObservation(revision core.BuildCommit, observation ObservationReference) error {
...
func (s *EvidenceSummary) addExperiment(observation ExperimentObservation) error {
	...
	case OutcomeSkipped, OutcomeUnavailable, OutcomeTimedOut, OutcomeCancelled, OutcomeSetupFailed, OutcomeInfrastructureFailed, OutcomeNotRun, OutcomeNonAccepting:
		s.NonAcceptingCount++
	case OutcomeUnknown:
		return contractError(errors.New("standard experiment summary received an unknown outcome"))
	default:
		return contractError(errors.New("standard experiment summary received an outcome outside its domain"))
	}
...
func (c ProjectCode) Validate() error {
	...
	if c.Unattributed.GoPackages != 0 || c.Unattributed.JavaScriptUnits != 0 || c.Unattributed.TestFiles != 0 || c.Unattributed.TestDeclarations != 0 || c.Unattributed.Benchmarks != 0 || c.Unattributed.FuzzTargets != 0 {
		return conflictError(errors.New("standard unattributed project code contains executable units"))
	}
	return nil
}
...
type Project struct {
	...
	Knowledge     ProjectKnowledge     `json:"knowledge"`
	SchemaVersion uint16               `json:"schema_version"`
	Revision      core.BuildCommit     `json:"revision"`
}
...
func (s Project) Validate() error {
	if !s.validShape() {
		return contractError(errors.New("standard project snapshot shape is invalid"))
	}
...
func (c Catalog) Validate() error {
	...
	if len(c.Packages) != len(c.Project.Packages) {
		return conflictError(errors.New("standard catalog package count differs from project index"))
	}
...
func (c Catalog) validateProjectCode() error {
	...
	if !total.matches(want) {
		return conflictError(errors.New("standard project code inventory differs from package inventories"))
	}
	return nil
}
...
func (c Catalog) validatePackage(index int) error {
	...
	if !sameSubject(current.Package.Subject, c.Project.Subject) || current.Package.Revision != c.Project.Revision {
		return conflictError(errors.New("standard package snapshot differs from project source"))
	}
	...
	if !packageSummaryEqual(summary, c.Project.Packages[index]) {
		return conflictError(errors.New("standard package summary differs from package snapshot"))
	}
	return nil
}
		...
		for componentIndex := range c.Packages[packageIndex].Code.Components {
			...
			for candidateIndex := range c.Packages {
				if candidateIndex != packageIndex && pathWithin(c.Packages[candidateIndex].Package.Knowledge.Path, path, true) {
					return conflictError(errors.New("standard component belongs to multiple packages"))
				}
			}
		}
	}
	...
	for _, capability := range c.Project.Capabilities {
		for _, contribution := range capability.Contributions {
			...
			if !ok || !featureExists(owner.Package.Knowledge.AuthorFeatures, contribution.FeatureID) {
				return conflictError(errors.New("standard capability contribution has no package feature"))
			}
			for _, surfaceID := range contribution.SurfaceIDs {
				if !surfaceExists(owner.Evidence.Surfaces, surfaceID) {
					return conflictError(errors.New("standard capability contribution has no evidence surface"))
				}
			}
		}
	}
	...
	for index := range values {
		...
		for previous := range index {
			if values[previous].ID == values[index].ID {
				return conflictError(errors.New("standard package group is duplicated"))
			}
		}
	}
	return nil
...
func validatePackageSummaries(groups []PackageGroup, values []PackageSummary) error {
	for index := range values {
		...
		if !groupExists(groups, values[index].GroupID) {
			return conflictError(errors.New("standard package summary has no package group"))
		}
		for previous := range index {
			if values[previous].Key == values[index].Key || values[previous].Path == values[index].Path {
				return conflictError(errors.New("standard package summary is duplicated"))
			}
		}
	}
	return nil
...
func validateCapabilities(features []Feature, values []ProjectCapability) error {
	if len(values) != len(features) || len(values) > FeatureMaximum {
		return conflictError(errors.New("standard project capabilities do not close over product features"))
	}
	for index := range values {
		if err := values[index].Validate(); err != nil {
			return err
		...
		if !featureExists(features, values[index].FeatureID) {
			return conflictError(errors.New("standard project capability names no product feature"))
		}
		for previous := range index {
			if values[previous].FeatureID == values[index].FeatureID {
				return conflictError(errors.New("standard project capability is duplicated"))
			}
		}
	}
	return nil
...
func findSurface(values []EvidenceSurface, id Identifier) (EvidenceSurface, bool) {
	for index := range values {
		if values[index].ID == id {
			return values[index], true
		}
	}
	return EvidenceSurface{}, false
}

func surfaceExists(values []EvidenceSurface, id Identifier) bool {
	_, ok := findSurface(values, id)
	return ok
}

func findRequest(values []RequestReference, id RequestIdentity) (RequestReference, bool) {
	for index := range values {
		if values[index].Request == id {
			return values[index], true
		}
	}
	return RequestReference{}, false
}
