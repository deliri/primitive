...
func (r *ProbeRole) UnmarshalJSON(data []byte) error {
	if r == nil {
		return jsonError(errors.New("nil standard probe role receiver"))
	}
...
func (k *ProbeKind) UnmarshalJSON(data []byte) error {
	if k == nil {
		return jsonError(errors.New("nil standard probe kind receiver"))
	}
...
func (k *ProbeTargetKind) UnmarshalJSON(data []byte) error {
	if k == nil {
		return jsonError(errors.New("nil standard probe target receiver"))
	}
...
func (t GoFileTarget) Validate() error {
	...
	if !strings.HasSuffix(t.File.String(), GoTestFileSuffix) {
		return contractError(errors.New("standard Go file selection requires a _test.go source file"))
	}
	return validateGoSelectionChildKinds(t.ChildKinds, "standard Go file child kinds are invalid")
}
...
func (t GoPackageTarget) Validate() error {
	...
	return validateGoSelectionChildKinds(t.ChildKinds, "standard Go package child kinds are invalid")
}
...
func (t ProbeTarget) Validate() error {
	...
	if t.payloadCount() != 1 {
		return contractError(errors.New("standard probe target must carry exactly one variant"))
	}
...
func validateTargetVariant[T interface{ Validate() error }](value *T) error {
	if value == nil {
		return conflictError(errors.New("standard probe target variant differs from kind"))
	}
...
func (p SelectionParent) Validate() error {
	...
	if role != ProbeRoleSelection || !targetAdmitsKind(p.Target.Kind, p.Kind) {
		return conflictError(errors.New("standard selection parent kind and target disagree"))
	}
	return nil
}
...
func (r RequestedProbe) Validate() error {
	...
	if r.Subject.Repository.value != r.Source.Repository.value {
		return conflictError(errors.New("standard requested probe source repository differs from subject"))
	}
...
func validateRequestedKinds(target ProbeTarget, kinds []ProbeKind) error {
	if len(kinds) == 0 || len(kinds) > ProbeKindMaximum {
		return contractError(errors.New("standard requested probe kinds are invalid"))
	}
	...
	if !requestedKindsMatchSelectionTarget(target, kinds) {
		return conflictError(errors.New("standard requested probe kinds differ from selection child kinds"))
	}
	return nil
}
...
func validateRequestedKindAt(target ProbeTarget, kinds []ProbeKind, index int) error {
	...
	if duplicateProbeKind(kinds, index) {
		return conflictError(errors.New("standard requested probe kind is duplicated"))
	}
	if index > 0 && kinds[index-1] >= kinds[index] {
		return conflictError(errors.New("standard requested probe kinds are not canonical"))
	}
	if !requestedTargetAdmitsKind(target.Kind, kinds[index]) {
		return conflictError(errors.New("standard requested probe kind differs from target"))
	}
	return nil
}
...
func AdmitRequestedProbe(request RequestedProbe, environment AdmittedEnvironment) (ProbeIdentity, error) {
	...
	if !environment.Satisfies(request.Constraints) {
		return ProbeIdentity{}, conflictError(errors.New("standard admitted environment does not satisfy the requested requirement"))
	}
...
func (i ProbeIdentity) Validate() error {
	...
	if i.Subject.Repository != i.Source.Repository {
		return conflictError(errors.New("standard probe identity source repository differs from subject"))
	}
...
func (i ProbeIdentity) validateRelationship() error {
	...
	if role != i.Role || !targetAdmitsKind(i.Target.Kind, i.Kind) {
		return conflictError(errors.New("standard probe role, kind, and target disagree"))
	}
	if i.Role == ProbeRoleSelection && i.Parent != nil {
		return conflictError(errors.New("standard selection probe cannot descend from another selection"))
	}
	if i.Parent != nil {
...
func (i ProbeIdentity) validateParent() error {
	...
	if !experimentDescendsFromSelection(i, *i.Parent) {
		return conflictError(errors.New("standard experiment does not descend from its selection parent"))
	}
	return nil
}
...
func validateGoSelectionChildKinds(values []ProbeKind, diagnostic string) error {
	...
	for index := range values {
		if !goDeclarationExperiment(values[index]) {
			return conflictError(errors.New("standard Go selection child kind is not a Go experiment"))
		}
		if duplicateProbeKind(values, index) || (index > 0 && values[index-1] >= values[index]) {
			return conflictError(errors.New("standard Go selection child kinds are not canonical"))
		}
	}
	return nil
}
