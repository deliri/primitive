...
// CountChange is a signed change in one bounded Standard count. Positive values
...
func (c CountChange) Validate() error {
	if c < -math.MaxUint32 || c > math.MaxUint32 {
		return contractError(errors.New("standard count change exceeds the uint32 source domain"))
	}
	return nil
}
...
func (c CoverageChange) Validate() error {
	if c.Before != nil && *c.Before > 10_000 || c.After != nil && *c.After > 10_000 {
		return contractError(errors.New("standard coverage change exceeds basis-point bounds"))
	}
	return nil
}

// InventoryChange makes the exact compiler-visible code-shape movement
// between two compatible package Standard snapshots explicit.
...
// Standard snapshot before and after a change. It retains source coordinates and
...
func (e PackageEvolution) Validate() error {
	...
	if len(e.NewReviewCandidates) > FunctionUsageMaximum || len(e.FormerReviewCandidates) > FunctionUsageMaximum {
		return contractError(errors.New("standard package evolution review candidates exceed their bound"))
	}
...
func ComparePackageSnapshots(before, after PackageSnapshot) (PackageEvolution, error) {
	...
	if !packageSnapshotsComparable(before, after) {
		return PackageEvolution{}, conflictError(errors.New("standard package snapshots do not identify the same package contract"))
	}
	if !sourceAnalysisComparable(before.Code.SourceUsage, after.Code.SourceUsage) {
		return PackageEvolution{}, conflictError(errors.New("standard package snapshots use different source-analysis generations"))
	}
