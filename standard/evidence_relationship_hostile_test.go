...
func TestExperimentOutcomeTerminalRelationshipExhaustive(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "unknown outcome never becomes a terminal fact", outcome: OutcomeUnknown, terminal: TerminalCompleted, wantErr: core.ErrStandardContract},
		...
		{name: "one fact terminal mutation cannot contradict passed outcome", outcome: OutcomePassed, terminal: TerminalFailed, wantErr: core.ErrStandardConflict},
	}
	...
	for _, current := range cases {
		t.Run(current.name, func(t *testing.T) {
			...
			if current.outcome == OutcomeUnknown {
				if mapped != TerminalUnknown || !errors.Is(mappingErr, core.ErrStandardContract) {
					t.Fatalf("TerminalForOutcome(%v) = (%v, %v), want (%v, errors.Is(..., %v))",
						current.outcome, mapped, mappingErr, TerminalUnknown, core.ErrStandardContract)
				}
...
func TestObservationAdmissionBindingRejectsOneFactTargetMutation(t *testing.T) {
	...
	if !errors.Is(gotErr, core.ErrStandardConflict) {
		t.Fatalf("ObservationReference.ValidateFor(mutated target) error = %v, want %v", gotErr, core.ErrStandardConflict)
	}
}
...
func TestRequestAdmissionBindingRejectsOneFactEnvironmentMutation(t *testing.T) {
	...
	if !errors.Is(gotErr, core.ErrStandardConflict) {
		t.Fatalf("RequestReference.ValidateFor(mutated admitted environment) error = %v, want %v", gotErr, core.ErrStandardConflict)
	}
}
...
func TestObservationIndependentAuthorityAndEnvironmentBindings(t *testing.T) {
	...
	t.Run("subject cannot verify its own observation", func(t *testing.T) {
		...
		if !errors.Is(gotErr, core.ErrStandardConflict) {
			t.Fatalf("ObservationReference.Validate(self verifier) error = %v, want %v", gotErr, core.ErrStandardConflict)
		}
	})
	...
	t.Run("experiment environment cannot differ from admitted probe", func(t *testing.T) {
		...
		gotErr := mutated.Validate()
		if !errors.Is(gotErr, core.ErrStandardConflict) {
			t.Fatalf("ObservationReference.Validate(mutated environment) error = %v, want %v", gotErr, core.ErrStandardConflict)
		}
	})
}
...
func fixtureAdmittedObservation(t testing.TB) (RequestReference, ObservationReference) {
	...
	target := ProbeTarget{
		...
		GoDeclaration: &GoDeclarationTarget{
			Module: fixtureIdentifier(t, "primitive"), Package: fixturePath(t, "standard"),
			File: fixturePath(t, "standard/evidence.go"), Symbol: fixtureName(t, "TestEvidence"),
		},
	}
