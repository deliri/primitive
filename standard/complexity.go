...
func complexityGrowthLabels() []string {
	return []string{"", sourceDeclarationConstantText, "logarithmic", "linear", "linearithmic", "polynomial", "exponential"}
}
func (g ComplexityGrowth) Validate() error {
	return validateEnum(uint8(g), complexityGrowthLabels(), complexityGrowthInvalidDiagnostic)
}
...
func (g ComplexityGrowth) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(g), complexityGrowthLabels(), complexityGrowthInvalidDiagnostic)
}
func (g *ComplexityGrowth) UnmarshalJSON(data []byte) error {
	if g == nil {
		return jsonError(errors.New("nil standard complexity growth receiver"))
	}
	value, err := unmarshalEnum(data, complexityGrowthLabels(), complexityGrowthInvalidDiagnostic)
	if err == nil {
...
func (c ComplexityCase) Validate() error {
	return validateEnum(uint8(c), complexityCaseLabels(), complexityCaseInvalidDiagnostic)
}
...
func (c ComplexityCase) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(c), complexityCaseLabels(), complexityCaseInvalidDiagnostic)
}
func (c *ComplexityCase) UnmarshalJSON(data []byte) error {
	if c == nil {
		return jsonError(errors.New("nil standard complexity case receiver"))
	}
	value, err := unmarshalEnum(data, complexityCaseLabels(), complexityCaseInvalidDiagnostic)
	if err == nil {
...
func (s ComplexityAssessmentStatus) Validate() error {
	return validateEnum(uint8(s), complexityAssessmentLabels(), complexityAssessmentInvalidDiagnostic)
}
...
func (s ComplexityAssessmentStatus) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(s), complexityAssessmentLabels(), complexityAssessmentInvalidDiagnostic)
}
func (s *ComplexityAssessmentStatus) UnmarshalJSON(data []byte) error {
	if s == nil {
		return jsonError(errors.New("nil standard complexity assessment receiver"))
	}
	value, err := unmarshalEnum(data, complexityAssessmentLabels(), complexityAssessmentInvalidDiagnostic)
	if err == nil {
...
func (b ComplexityBound) Validate() error {
	...
	if b.Growth == ComplexityPolynomial {
		if b.Degree < 2 || b.Degree > ComplexityPolynomialMaximum {
			return contractError(errors.New("standard complexity polynomial degree is invalid"))
		}
		return nil
	}
	if b.Degree != 0 {
		return conflictError(errors.New("standard non-polynomial complexity carries a degree"))
	}
	return nil
}
...
func (i ComplexityInput) Validate() error {
	...
	if !i.validRange() {
		return contractError(errors.New("standard complexity input range is invalid"))
	}
	for index := range i.SampleSizes {
		if i.SampleSizes[index] < i.Minimum || i.SampleSizes[index] > i.Maximum {
			return conflictError(errors.New("standard complexity sample is outside its input range"))
		}
		if index > 0 && i.SampleSizes[index] <= i.SampleSizes[index-1] {
			return conflictError(errors.New("standard complexity sample sizes are not strictly increasing"))
		}
	}
	return nil
}
...
func (c ComplexityClaim) Validate() error {
	if c.Operation.Symbol == nil || len(c.Assumptions) == 0 || len(c.Assumptions) > ComplexityAssumptionMaximum {
		return contractError(errors.New("standard complexity claim is incomplete"))
	}
...
func (s ComplexitySample) Validate() error {
	if s.InputSize == 0 || s.Iterations == 0 || s.NanosecondsPerOp == 0 {
		return contractError(errors.New("standard complexity sample is invalid"))
	}
	return nil
}
...
func (c ComplexityCapture) Validate() error {
	if c.BudgetSeconds == 0 || len(c.Samples) < ComplexitySampleMinimum || len(c.Samples) > ComplexitySampleMaximum {
		return contractError(errors.New("standard complexity capture bounds are invalid"))
	}
	...
	for index := range c.Samples {
		...
		if index > 0 && c.Samples[index].InputSize <= c.Samples[index-1].InputSize {
			return conflictError(errors.New("standard complexity captures are not strictly increasing"))
		}
	}
	return nil
}
