...
type standardsJSONDoor uint8

const (
	doorIdentifier standardsJSONDoor = iota
	doorName
	doorText
	doorSourcePath
	...
	doorFunctionPosture
	standardsJSONDoorLimit
)

type comparableStandardJSON interface {
	comparable
	core.ValidatedJSONMarshaler
}

func FuzzStandardTextConstructorsSemanticClosure(f *testing.F) {
	f.Add(uint8(0), "identity")
	f.Add(uint8(1), "Package name")
	f.Add(uint8(2), "Exact engineering knowledge.")
	f.Add(uint8(3), "standard/catalog.go")
	...
	f.Fuzz(func(t *testing.T, selector uint8, value string) {
		...
		default:
			t.Fatalf("Standard text constructor selector = %d, want closed 0..4", selector%5)
		}
	})
}

func FuzzStandardAtomicJSONDoorsSemanticClosure(f *testing.F) {
	...
	addAtomicDoorSeed(f, doorSourcePath, fixturePath(f, "standard/catalog.go"))
	...
	f.Fuzz(func(t *testing.T, rawDoor uint8, data []byte) {
		door := standardsJSONDoor(rawDoor % uint8(standardsJSONDoorLimit))
		switch door {
		case doorIdentifier:
		...
		case doorSourcePath:
			proveComparableJSONClosure(t, fixturePath(t, "standard/catalog.go"), data, (*SourcePath).UnmarshalJSON)
		...
		default:
			t.Fatalf("Standard JSON door = %d, want a closed admitted door", door)
		}
	})
}

func FuzzPackageRoleDeclarationJSONSemanticClosure(f *testing.F) {
	seed := PackageRoleDeclaration{Path: fixturePath(f, "standard"), Role: core.PackageRoleDomainAgreement}
	addDocumentSeed(f, uint8(0), seed)
	f.Add(uint8(0), []byte{})
	f.Add(uint8(0), []byte(`{}`))
	f.Add(uint8(0), []byte(`null`))
	f.Add(uint8(0), []byte(`{"path":"standard","role":"domain_agreement","role":"wire_protocol"}`))

	f.Fuzz(func(t *testing.T, _ uint8, data []byte) {
		proveDocumentJSONClosure(t, seed, data, (*PackageRoleDeclaration).UnmarshalJSON)
	})
}

func FuzzStandardMachineProbeReportJSONSemanticClosure(f *testing.F) {
...
func addAtomicDoorSeed[T comparableStandardJSON](f *testing.F, door standardsJSONDoor, seed T) {
	f.Helper()
	encoded, err := seed.MarshalJSON()
	if err != nil {
		f.Fatalf("Standard atomic JSON seed MarshalJSON() error = %v, want nil", err)
	}
	f.Add(uint8(door), encoded)
}

func addEnumDoorSeeds[T comparableStandardJSON](f *testing.F, door standardsJSONDoor, limit uint8, construct func(uint8) T) {
	f.Helper()
...
func addDocumentSeed[T core.ValidatedJSONMarshaler](f *testing.F, selector uint8, seed T) {
	...
	encoded, err := seed.MarshalJSON()
	if err != nil {
		f.Fatalf("Standard document seed MarshalJSON() error = %v, want nil", err)
	}
	f.Add(selector, encoded)
}

func proveConstructedDoor[T comparableStandardJSON](t *testing.T, seed T, constructErr error, data []byte, unmarshal func(*T, []byte) error) {
	t.Helper()
	if constructErr != nil {
		t.Fatalf("Standard identity seed construction error = %v, want nil", constructErr)
	}
	proveComparableJSONClosure(t, seed, data, unmarshal)
}

func proveTextConstructorClosure[T comparableStandardJSON](t *testing.T, value string, construct func(string) (T, error), unmarshal func(*T, []byte) error) {
	t.Helper()
	got, gotErr := construct(value)
	if gotErr != nil {
		var zero T
		if !errors.Is(gotErr, core.ErrStandardContract) || got != zero {
			t.Fatalf("Standard text constructor(rejected) = (%v, %v), want (zero, errors.Is(..., %v))", got, gotErr, core.ErrStandardContract)
		}
		return
	}
	if err := got.Validate(); err != nil {
		t.Fatalf("Standard text constructor(accepted).Validate() error = %v, want nil", err)
	}
	encoded, err := got.MarshalJSON()
	if err != nil || len(encoded) > core.JSONDocumentMaximumBytes {
		t.Fatalf("Standard text constructor(accepted).MarshalJSON() = (%d bytes, %v), want bounded and nil", len(encoded), err)
	}
	var roundTrip T
	if err := unmarshal(&roundTrip, encoded); err != nil || roundTrip != got {
		t.Fatalf("Standard text constructor JSON round trip = (%v, %v), want (%v, nil)", roundTrip, err, got)
	}
}

func proveComparableJSONClosure[T comparableStandardJSON](t *testing.T, seed T, data []byte, unmarshal func(*T, []byte) error) {
	t.Helper()
	got := seed
	gotErr := unmarshal(&got, data)
	if gotErr != nil {
		if !errors.Is(gotErr, core.ErrJSONContract) || !errors.Is(gotErr, core.ErrStandardContract) || got != seed {
			t.Fatalf("Standard atomic UnmarshalJSON(rejected) = (%v, %v), want preserved and typed JSON/Standard rejection", got, gotErr)
		}
		return
	}
	if err := got.Validate(); err != nil {
		t.Fatalf("Standard atomic UnmarshalJSON(accepted).Validate() error = %v, want nil", err)
	}
	encoded, err := got.MarshalJSON()
	if err != nil || len(encoded) > core.JSONDocumentMaximumBytes {
		t.Fatalf("Standard atomic MarshalJSON(accepted) = (%d bytes, %v), want bounded and nil", len(encoded), err)
	}
	var roundTrip T
	if err := unmarshal(&roundTrip, encoded); err != nil || roundTrip != got {
		t.Fatalf("Standard atomic canonical round trip = (%v, %v), want (%v, nil)", roundTrip, err, got)
	}
	second, err := roundTrip.MarshalJSON()
	if err != nil || !bytes.Equal(second, encoded) {
		t.Fatalf("Standard atomic second canonical projection = (%q, %v), want (%q, nil)", second, err, encoded)
	}
}
...
func proveDocumentJSONClosure[T core.ValidatedJSONMarshaler](t *testing.T, seed T, data []byte, unmarshal func(*T, []byte) error) {
	...
	if wantErr != nil {
		t.Fatalf("Standard document seed MarshalJSON() setup error = %v, want nil", wantErr)
	}
	got := seed
	gotErr := unmarshal(&got, data)
	if gotErr != nil {
		preserved, preservedErr := got.MarshalJSON()
		if !errors.Is(gotErr, core.ErrJSONContract) || !errors.Is(gotErr, core.ErrStandardContract) || preservedErr != nil || !bytes.Equal(preserved, wantPreserved) {
			t.Fatalf("Standard document UnmarshalJSON(rejected) = (%d preserved bytes, %v, %v), want exact preserved seed and typed JSON/Standard rejection", len(preserved), preservedErr, gotErr)
		}
		return
	}
	if err := got.Validate(); err != nil {
		t.Fatalf("Standard document UnmarshalJSON(accepted).Validate() error = %v, want nil", err)
	}
	encoded, err := got.MarshalJSON()
	if err != nil || len(encoded) > core.JSONDocumentMaximumBytes {
		t.Fatalf("Standard document MarshalJSON(accepted) = (%d bytes, %v), want bounded and nil", len(encoded), err)
	}
	var roundTrip T
	if err := unmarshal(&roundTrip, encoded); err != nil {
		t.Fatalf("Standard document canonical round trip error = %v, want nil", err)
	}
	second, err := roundTrip.MarshalJSON()
	if err != nil || !bytes.Equal(second, encoded) {
		t.Fatalf("Standard document second canonical projection = (%q, %v), want (%q, nil)", second, err, encoded)
	}
}

func fixtureStandardUUID(t testing.TB) primitiveid.UUIDv7 {
	t.Helper()
