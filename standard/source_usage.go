...
func sourceAnalysisLabels() []string {
	return []string{"", "analysis_complete", "analysis_" + canonicalUnresolvedText}
}
func (c SourceAnalysisCompleteness) Validate() error {
	return validateEnum(uint8(c), sourceAnalysisLabels(), sourceAnalysisCompletenessInvalidDiagnostic)
}
...
func (c SourceAnalysisCompleteness) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(c), sourceAnalysisLabels(), sourceAnalysisCompletenessInvalidDiagnostic)
}
func (c *SourceAnalysisCompleteness) UnmarshalJSON(data []byte) error {
	if c == nil {
		return jsonError(errors.New("nil standard source analysis receiver"))
	}
	value, err := unmarshalEnum(data, sourceAnalysisLabels(), sourceAnalysisCompletenessInvalidDiagnostic)
	if err == nil {
...
func functionReferenceLabels() []string {
	return []string{"", "no_reference", "test_reference", canonicalUnresolvedText, "production_reference", "runtime_entry_point"}
}
func (p FunctionReferencePosture) Validate() error {
	return validateEnum(uint8(p), functionReferenceLabels(), functionReferencePostureInvalidDiagnostic)
}
...
func (p FunctionReferencePosture) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(p), functionReferenceLabels(), functionReferencePostureInvalidDiagnostic)
}
func (p *FunctionReferencePosture) UnmarshalJSON(data []byte) error {
	if p == nil {
		return jsonError(errors.New("nil standard function posture receiver"))
	}
	value, err := unmarshalEnum(data, functionReferenceLabels(), functionReferencePostureInvalidDiagnostic)
	if err == nil {
...
func (u FunctionUsage) Validate() error {
	if len(u.ObservedConsumers) > ObservedConsumerMaximum || u.Function.Symbol == nil || u.DeclarationLine == 0 {
		return contractError(errors.New("standard function usage shape is invalid"))
	}
	if err := contractJoin(u.Function.Validate(), u.ReferencePosture.Validate()); err != nil {
		return err
	}
	if !functionUsageCountsMatch(u) {
		return conflictError(errors.New("standard function usage counts disagree with posture"))
	}
	for index := range u.ObservedConsumers {
		...
		for previous := range index {
			if u.ObservedConsumers[previous] == u.ObservedConsumers[index] {
				return conflictError(errors.New("standard function consumer is duplicated"))
			}
		}
	}
	return nil
...
func (u PackageSourceUsage) Validate() error {
	if len(u.ObservedConsumerPackages) > ObservedConsumerMaximum || len(u.MostReferenced) > FunctionUsageMaximum || len(u.ReviewCandidates) > FunctionUsageMaximum {
		return contractError(errors.New("standard package source usage bounds are invalid"))
	}
	...
	if u.DeclarationCount != u.ProductionReferenced+u.RuntimeEntryPoints+u.UnresolvedDeclarations+u.TestReferencedOnly+u.NoReferenceObserved {
		return conflictError(errors.New("standard package source usage accounting does not close"))
	}
	if (u.Completeness == SourceAnalysisComplete) != (u.UnresolvedDeclarations == 0) {
		return conflictError(errors.New("standard source completeness disagrees with unresolved declarations"))
	}
	...
	for index := range values {
		...
		for previous := range index {
			if values[previous] == values[index] {
				return conflictError(errors.New("standard source path is duplicated"))
			}
		}
	}
	return nil
...
func validateFunctionUsages(owner SourcePath, values []FunctionUsage) error {
	for index := range values {
		...
		if !pathWithin(owner, values[index].Function.Path, true) {
			return conflictError(errors.New("standard function usage is outside its package"))
		}
		for previous := range index {
			if codeReferenceEqual(values[previous].Function, values[index].Function) {
				return conflictError(errors.New("standard function usage is duplicated"))
			}
		}
	}
	return nil
