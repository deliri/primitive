...
func validatePackageRoleOwnership(role core.PackageRole, ownership []CapabilityOwnership) error {
	...
	if role == core.PackageRoleEffectCapability && !hasOwnership {
		return conflictError(errors.New("standard effect capability lacks authored capability ownership"))
	}
	if role != core.PackageRoleEffectCapability && hasOwnership {
		return conflictError(errors.New("standard non-effect package declares capability ownership"))
	}
	return nil
}

const (
	capabilityRoleInvalidDiagnostic      = "standard capability role is invalid"
	projectRelationshipInvalidDiagnostic = "standard project relationship is invalid"
)
...
func capabilityRoleLabels() []string {
	return []string{"", primitiveImplementationText, capabilityDeclaredAdapterText}
}
...
func (r *CapabilityRole) UnmarshalJSON(data []byte) error {
	if r == nil {
		return jsonError(errors.New("nil standard capability role receiver"))
	}
...
func projectRelationshipLabels() []string {
	return []string{"", canonicalPrimitiveText, canonicalProductText, capabilityDeclaredAdapterText}
}
...
func (r *ProjectRelationship) UnmarshalJSON(data []byte) error {
	if r == nil {
		return jsonError(errors.New("nil standard project relationship receiver"))
	}
...
func validateCapabilityOwnership(values []CapabilityOwnership) error {
	if len(values) > capabilities.IdentityCount {
		return contractError(errors.New("standard capability ownership exceeds its bound"))
	}
	for index := range values {
		...
		for previous := range index {
			if values[previous].Capability == values[index].Capability {
				return conflictError(errors.New("standard capability ownership is duplicated"))
			}
		}
	}
	return nil
