...
func (u Usage) Validate() error {
	if len(u.Steps) == 0 || len(u.Steps) > UsageStepMaximum {
		return contractError(errors.New("standard usage step count is invalid"))
	}
...
func (s AssuranceStage) Validate() error {
	return validateEnum(uint8(s), assuranceStageLabels(), assuranceStageInvalidDiagnostic)
}
...
func (s AssuranceStage) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(s), assuranceStageLabels(), assuranceStageInvalidDiagnostic)
}
func (s *AssuranceStage) UnmarshalJSON(data []byte) error {
	if s == nil {
		return jsonError(errors.New("nil standard assurance stage receiver"))
	}
	value, err := unmarshalEnum(data, assuranceStageLabels(), assuranceStageInvalidDiagnostic)
	if err == nil {
...
func assuranceAuthorityLabels() []string {
	return []string{"", canonicalProductText, core.PackageCore.String(), canonicalPrimitiveText, "independent"}
}
func (a AssuranceAuthority) Validate() error {
	return validateEnum(uint8(a), assuranceAuthorityLabels(), assuranceAuthorityInvalidDiagnostic)
}
...
func (a AssuranceAuthority) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(a), assuranceAuthorityLabels(), assuranceAuthorityInvalidDiagnostic)
}
func (a *AssuranceAuthority) UnmarshalJSON(data []byte) error {
	if a == nil {
		return jsonError(errors.New("nil standard assurance authority receiver"))
	}
	value, err := unmarshalEnum(data, assuranceAuthorityLabels(), assuranceAuthorityInvalidDiagnostic)
	if err == nil {
...
func (c AssuranceControl) Validate() error {
	if len(c.References) == 0 || len(c.References) > AssuranceReferenceMaximum || len(c.SurfaceIDs) == 0 || len(c.SurfaceIDs) > AssuranceSurfaceMaximum {
		return contractError(errors.New("standard assurance control bounds are invalid"))
	}
	...
	for index := range c.References {
		...
		for previous := range index {
			if codeReferenceEqual(c.References[previous], c.References[index]) {
				return conflictError(errors.New("standard assurance reference is duplicated"))
			}
		}
	}
...
func (a Assurance) Validate() error {
	...
	for index := range controls {
		...
		if controls[index].Stage != stages[index] {
			return conflictError(errors.New("standard assurance control occupies the wrong stage"))
		}
	}
	return nil
}
...
func (k ComponentKind) Validate() error {
	return validateEnum(uint8(k), componentKindLabels(), componentKindInvalidDiagnostic)
}
...
func (k ComponentKind) MarshalJSON() ([]byte, error) {
	return marshalEnum(uint8(k), componentKindLabels(), componentKindInvalidDiagnostic)
}
func (k *ComponentKind) UnmarshalJSON(data []byte) error {
	if k == nil {
		return jsonError(errors.New("nil standard component kind receiver"))
	}
	value, err := unmarshalEnum(data, componentKindLabels(), componentKindInvalidDiagnostic)
	if err == nil {
...
func (c Component) Validate() error {
	...
	if !pathWithin(c.Package, c.Path, false) {
		return conflictError(errors.New("standard component is outside its package"))
	}
...
func (p ProjectKnowledge) Validate() error {
...
func (i Inventory) Validate() error {
	if i.Files == 0 || i.GoPackages > i.Files || i.JavaScriptUnits > i.Files || i.TestFiles > i.Files || i.Documents > i.Files {
		return contractError(errors.New("standard inventory file accounting is invalid"))
	}
	if uint64(i.TestDeclarations) > uint64(i.TestFiles)*SourceFileDeclarationMaximum || uint64(i.Benchmarks)+uint64(i.FuzzTargets) > uint64(i.TestDeclarations) {
		return conflictError(errors.New("standard inventory declaration accounting is invalid"))
	}
	if i.CoverageBasisPoints != nil && *i.CoverageBasisPoints > 10_000 {
		return contractError(errors.New("standard inventory coverage is invalid"))
	}
	return nil
}
...
func validateKnowledgeLists(lists knowledgeLists) error {
	if !validKnowledgeListBounds(lists) {
		return contractError(errors.New("standard authored knowledge list bounds are invalid"))
	}
	...
	for index := range values {
		...
		for previous := range index {
			if values[previous].ID == values[index].ID {
				return conflictError(errors.New("standard feature identity is duplicated"))
			}
		}
	}
	return nil
...
func validateUsage(values []Usage) error {
	if len(values) == 0 || len(values) > UsageMaximum {
		return contractError(errors.New("standard usage count is invalid"))
	}
	for index := range values {
		if err := values[index].Validate(); err != nil {
			return err
		...
		for previous := range index {
			if values[previous].ID == values[index].ID {
				return conflictError(errors.New("standard usage identity is duplicated"))
			}
		}
	}
	return nil
...
func validateComplexityClaims(owner SourcePath, values []ComplexityClaim) error {
	if len(values) > ComplexityClaimMaximum {
		return contractError(errors.New("standard complexity claim count is invalid"))
	}
	for index := range values {
		if err := values[index].Validate(); err != nil {
			return err
		...
		if !pathWithin(owner, values[index].Operation.Path, true) {
			return conflictError(errors.New("standard complexity operation is outside its package"))
		}
		for previous := range index {
			if values[previous].ID == values[index].ID {
				return conflictError(errors.New("standard complexity claim identity is duplicated"))
			}
		}
	}
	return nil
