...
func (k *DispositionKind) UnmarshalJSON(data []byte) error {
	if k == nil {
		return jsonError(errors.New("nil standard disposition receiver"))
	}
...
func (r *RefusalReason) UnmarshalJSON(data []byte) error {
	if r == nil {
		return jsonError(errors.New("nil standard refusal reason receiver"))
	}
...
func (s *TerminalState) UnmarshalJSON(data []byte) error {
	if s == nil {
		return jsonError(errors.New("nil standard terminal state receiver"))
	}
...
func (k *ObservationKind) UnmarshalJSON(data []byte) error {
	if k == nil {
		return jsonError(errors.New("nil standard observation kind receiver"))
	}
...
func (s *InfrastructureStage) UnmarshalJSON(data []byte) error {
	if s == nil {
		return jsonError(errors.New("nil standard infrastructure stage receiver"))
	}
...
func (s EvidenceSurface) Validate() error {
	...
	if len(s.EligibleKinds) == 0 || len(s.EligibleKinds) > ProbeKindMaximum || len(s.Profiles) == 0 || len(s.Profiles) > ProbeKindMaximum || len(s.ComplexityClaimIDs) > ComplexityClaimMaximum {
		return contractError(errors.New("standard evidence surface bounds are invalid"))
	}
...
func validateSurfaceKinds(target ProbeTarget, kinds []ProbeKind) error {
	for index := range kinds {
		...
		if !requestedTargetAdmitsKind(target.Kind, kinds[index]) || duplicateProbeKind(kinds, index) {
			return conflictError(errors.New("standard evidence surface kind is contradictory"))
		}
		if index > 0 && kinds[index-1] >= kinds[index] {
			return conflictError(errors.New("standard evidence surface kinds are not canonical"))
		}
	}
	if !requestedKindsMatchSelectionTarget(target, kinds) {
		return conflictError(errors.New("standard evidence surface kinds differ from selection child kinds"))
	}
	return nil
}
	...
	for index := range values {
		...
		for previous := range index {
			if values[previous] == values[index] {
				return conflictError(errors.New("standard profile identity is duplicated"))
			}
		}
	}
	return nil
	...
	for index := range values {
		...
		for previous := range index {
			if values[previous].value == values[index].value {
				return conflictError(errors.New("standard identifier is duplicated"))
			}
		}
	}
	return nil
...
func (d RequestDisposition) Validate() error {
	...
	if (d.Admitted != nil) == (d.Refused != nil) {
		return contractError(errors.New("standard disposition must carry exactly one variant"))
	}
	...
	return conflictError(errors.New("standard disposition payload differs from kind"))
}
...
func (r RequestReference) ValidateFor(surface EvidenceSurface) error {
	...
	if r.SurfaceID.value != surface.ID.value || !sameSubject(r.Requested.Subject, surface.Subject) || !sameTarget(r.Requested.Target, surface.Target) || r.Source != r.Requested.Source {
		return conflictError(errors.New("standard request does not descend from its evidence surface"))
	}
	if !requestedKindsAllowed(r.Requested.Kinds, surface.EligibleKinds) || !profileAllowed(r.Requested.Profile, surface.Profiles) {
		return conflictError(errors.New("standard request exceeds its evidence surface"))
	}
...
func validateAdmissionForRequest(admission Admission, requested RequestedProbe) error {
	if !sameSubject(admission.Probe.Subject, requested.Subject) || admission.Probe.Origin != requested.Origin || admission.Probe.Source != requested.Source || !sameTarget(admission.Probe.Target, requested.Target) || admission.Probe.Profile != requested.Profile || !admission.Probe.Environment.Satisfies(requested.Constraints) || admission.Probe.Parent != nil {
		return conflictError(errors.New("standard admission differs from requested probe"))
	}
	...
	if admission.Probe.Kind != want {
		return conflictError(errors.New("standard admitted probe kind differs from requested target"))
	}
	return nil
}
...
func admittedKindForRequest(requested RequestedProbe) (ProbeKind, error) {
	if len(requested.Kinds) == 0 {
		return ProbeKindUnknown, contractError(errors.New("standard requested probe has no kind"))
	}
	...
	case ProbeTargetGoDeclaration:
		return soleRequestedKind(requested, "standard direct declaration admission requires one requested kind")
...
func (m DecimalMeasurement) Validate() error {
	if m.Coefficient == 0 || m.Scale > DecimalMeasurementScaleMaximum {
		return contractError(errors.New("standard decimal measurement is invalid"))
	}
	if m.Scale > 0 && m.Coefficient%10 == 0 {
		return contractError(errors.New("standard decimal measurement is not canonical"))
	}
	return nil
}
...
func (m BenchmarkMeasurement) Validate() error {
	...
	if m.Iterations == 0 {
		return contractError(errors.New("standard benchmark measurement is invalid"))
	}
...
func (p *CachePosture) UnmarshalJSON(data []byte) error {
	if p == nil {
		return jsonError(errors.New("nil standard cache posture receiver"))
	}
...
func (a ExecutionAttempt) Validate() error {
	...
	if a.Sequence == 0 || a.Planned == 0 {
		return contractError(errors.New("standard execution attempt sequence or planned denominator is zero"))
	}
	...
	if total != uint64(a.Planned) {
		return conflictError(errors.New("standard execution attempt outcome counts do not close against its planned denominator"))
	}
	return nil
}
...
func (a ExecutionAccounting) Validate() error {
	if len(a.Attempts) == 0 || len(a.Attempts) > ExecutionAttemptMaximum {
		return contractError(errors.New("standard execution attempt sequence is empty or exceeds its bound"))
	}
	for index := range a.Attempts {
		...
		if a.Attempts[index].Sequence != wantSequence {
			return conflictError(errors.New("standard execution attempts are not in canonical append-only sequence"))
		}
		if index > 0 && (a.Attempts[index].Planned != a.Attempts[0].Planned || a.Attempts[index].Filtered != a.Attempts[0].Filtered) {
			return conflictError(errors.New("standard execution retry changed the planned denominator or filter scope"))
		}
	}
	return nil
}
...
func (m ExperimentMeasurements) Validate() error {
	if m.CoverageBasisPoints != nil && *m.CoverageBasisPoints > 10_000 {
		return contractError(errors.New("standard coverage exceeds one hundred percent"))
	}
	if len(m.Benchmarks) > BenchmarkMeasurementMaximum || len(m.Complexity) > ComplexityCaptureMaximum {
		return contractError(errors.New("standard measurement collection exceeds its bound"))
	}
	if m.Accounting != nil {
...
func (o SelectionObservation) Validate() error {
	...
	if o.Admitted+o.Refused != o.Planned || o.Executed+o.NotRun != o.Admitted {
		return conflictError(errors.New("standard selection accounting does not close"))
	}
	return nil
}
...
func (o ExperimentObservation) Validate() error {
	if len(o.Artifacts) > ArtifactReferenceMaximum {
		return contractError(errors.New("standard experiment artifact count exceeds its bound"))
	}
...
func (o ExperimentObservation) validateExecutionState() error {
	if o.Started {
		if o.Measurements.DurationNs == 0 || o.Outcome == OutcomeNotRun {
			return conflictError(errors.New("standard started experiment has contradictory execution facts"))
		}
		return nil
	}
	if !o.Measurements.isZero() || len(o.Artifacts) != 0 || !outcomePermitsUnstarted(o.Outcome) {
		return conflictError(errors.New("standard unstarted experiment has contradictory execution facts"))
	}
	return nil
}
...
func (o InfrastructureObservation) Validate() error {
	if len(o.PartialEvidence) > ArtifactReferenceMaximum {
		return contractError(errors.New("standard partial evidence exceeds its bound"))
	}
	...
	for index := range values {
		...
		for previous := range index {
			if values[previous].Path.value == values[index].Path.value {
				return conflictError(errors.New("standard artifact path is duplicated"))
			}
		}
	}
	return nil
...
func (r ObservationReference) Validate() error {
	...
	if !r.independentAuthorities() {
		return conflictError(errors.New("standard evidence authorities are not independent of requester and subject"))
	}
	if r.payloadCount() != 1 {
		return contractError(errors.New("standard observation must carry exactly one variant"))
	}
...
func (r ObservationReference) validateExperimentPayload() error {
	...
	if !experimentTerminalMatches(r.Experiment.Outcome, r.Terminal) {
		return conflictError(errors.New("standard experiment outcome and terminal state disagree"))
	}
	return nil
}
...
func (r ObservationReference) validateExperimentBinding() error {
	if r.Experiment.EnvironmentFingerprint != r.Probe.Environment.EnvironmentFingerprint || r.Experiment.MachineSheetDigest != r.Probe.Environment.MachineSheetDigest {
		return conflictError(errors.New("standard experiment environment differs from admitted probe"))
	}
	for _, capture := range r.Experiment.Measurements.Complexity {
		if capture.EnvironmentFingerprint != r.Experiment.EnvironmentFingerprint || capture.Profile != r.Probe.Profile {
			return conflictError(errors.New("standard complexity capture differs from experiment identity"))
		}
	}
	return nil
}
...
func TerminalForOutcome(outcome Outcome) (TerminalState, error) {
	...
	return TerminalUnknown, contractError(errors.New("standard experiment outcome has no terminal state"))
}
...
func (r ObservationReference) ValidateFor(request RequestReference) error {
	...
	if request.Disposition.Admitted == nil {
		return conflictError(errors.New("standard observation descends from a refused request"))
	}
	admission := request.Disposition.Admitted
	if !r.matchesAdmission(request, *admission) {
		return conflictError(errors.New("standard observation does not descend from its admission"))
	}
	if r.Probe.Source != r.Source {
		return conflictError(errors.New("standard observation probe source differs from observation source"))
	}
	if r.Probe.Parent != nil && !r.matchesParent(*admission) {
		return conflictError(errors.New("standard observation child differs from admitted selection"))
	}
	return nil
}
