...
type standardsJSONDoorInventory struct {
	...
	PackageRoleDeclaration     func(*PackageRoleDeclaration, []byte) error
	MachineProbeReport         func(*MachineProbeReport, []byte) error
}

var standardsJSONDoors = standardsJSONDoorInventory{
	...
	PackageRoleDeclaration:     (*PackageRoleDeclaration).UnmarshalJSON,
...
func TestStandardDataFlowStructInventoryRatchet(t *testing.T) {
	t.Parallel()

	gotStructs, gotMarkers, gotErr := scanStandardDataFlowInventory()
	if gotErr != nil {
		t.Fatalf("scanStandardDataFlowInventory() error = %v, want nil", gotErr)
	}
	if !slices.Equal(gotStructs, gotMarkers) {
		t.Fatalf("Standard production structs = %q, want exactly one classified marker each; markers = %q", gotStructs, gotMarkers)
	}
}

func TestStandardExternalIngressInventoryMatchesSemanticFuzzDoors(t *testing.T) {
	t.Parallel()

	gotJSON, gotText, gotErr := scanStandardExternalDoors()
	if gotErr != nil {
		t.Fatalf("scanStandardExternalDoors() error = %v, want nil", gotErr)
	}
	wantJSON := inventoryFieldNames(standardsJSONDoors)
	wantText := inventoryFieldNames(aboutTextDoors)
	if !slices.Equal(gotJSON, wantJSON) {
		t.Fatalf("Standard production JSON doors = %q, want semantic fuzz inventory %q", gotJSON, wantJSON)
	}
	if !slices.Equal(gotText, wantText) {
		t.Fatalf("Standard production text doors = %q, want semantic fuzz inventory %q", gotText, wantText)
	}
	if len(wantJSON) != int(standardsJSONDoorLimit)+2 {
		t.Fatalf("Standard JSON fuzz selectors = %d atomic + 2 documents, want %d inventoried doors", standardsJSONDoorLimit, len(wantJSON))
	}
}

func scanStandardDataFlowInventory() ([]string, []string, error) {
...
func scanStandardExternalDoors() ([]string, []string, error) {
	entries, err := aboutContractSources.ReadDir(".")
	if err != nil {
		return nil, nil, err
	}
...
func aboutRoleMarker(name string) bool {
	switch name {
	case "standardsProtocolFact", "standardsSealedProjection", "standardsInternalFlowCarrier", "standardsCapabilityWrapper":
		return true
	default:
		return false
	}
