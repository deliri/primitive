...
func TestCurrentMachineDigestBindsTheCompleteValidatedObservation(t *testing.T) {
	...
	if !errors.Is(invalidErr, core.ErrStandardContract) || invalidDigest != (core.SHA256Digest{}) {
		t.Fatalf("CurrentMachine.Digest(invalid) = (%v, %v), want zero and errors.Is(..., %v)", invalidDigest, invalidErr, core.ErrStandardContract)
	}
}
