...
// The finite fuzz fixture budget is not a production transfer limit.
const filestoreFuzzPayloadFixtureBytes = 1 << 20
...
func FuzzWriteReadRoundTrip(f *testing.F) {
	...
	f.Add(emitted)
	for _, payload := range [][]byte{{}, deterministicPayload(filestoreFuzzPayloadFixtureBytes - 1), deterministicPayload(filestoreFuzzPayloadFixtureBytes), deterministicPayload(filestoreFuzzPayloadFixtureBytes + 1)} {
		f.Add(payload)
	}
	f.Fuzz(func(t *testing.T, payload []byte) {
		payload = payload[:min(len(payload), filestoreFuzzPayloadFixtureBytes+1)]
		directory := t.TempDir()
		...
		recovery, gotErr := filestore.Write(t.Context(), filestore.WriteRequest{Source: source, Location: location, Temporary: mustRelativePath(t, "stage"), Mode: 0o600, Install: filestore.InstallCreate})
		wantEntries := 2
		if gotErr != nil || recovery != (filestore.CommitRequest{}) || source.Len() != 0 {
			t.Fatalf("Write = (%+v,%v,%d unread), want zero recovery, nil error, consumed full input", recovery, gotErr, source.Len())
		}
...
func FuzzStageCommitRoundTrip(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, payload []byte) {
		...
		staged, gotErr := filestore.Stage(t.Context(), filestore.StageRequest{Source: source, Temporary: filestore.Location{Root: root, Path: stagePath}, Mode: 0o600})
		wantEntries := 2
		if gotErr != nil || source.Len() != 0 {
			t.Fatalf("Stage = (%+v,%v,%d unread), want nil error and consumed full input", staged, gotErr, source.Len())
		}
