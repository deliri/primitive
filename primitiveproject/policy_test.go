...
const standardSourcePath = "primitiveproject/policy_test.go"

// StandardKnowledge returns Primitive's compiler-owned authored
// project meaning. The caller supplies Git origins because history is an
// observed source fact rather than authored policy.
func StandardKnowledge(created standard.OptionalGitOrigin, changed standard.GitOrigin) (standard.ProjectKnowledge, error) {
	builder := standardBuilder{}
	knowledge := standard.ProjectKnowledge{
		Created:        created,
		Changed:        changed,
		...
		SourcePath:     builder.path(standardSourcePath),
		AuthorReasons: []standard.Reason{
		...
		AuthorNonGoals: []standard.Boundary{
		...
		AuthorFeatures: []standard.Feature{
			{
				...
				Delivery:         standard.DeliveryDelivered,
			},
			{
	...
	if builder.err != nil {
		return standard.ProjectKnowledge{}, builder.err
	}
	if err := knowledge.Validate(); err != nil {
		return standard.ProjectKnowledge{}, err
	}
...
func (b *standardBuilder) identifier(value string) standard.Identifier {
	result, err := standard.NewIdentifier(value)
	b.err = errors.Join(b.err, err)
	return result
}

func (b *standardBuilder) name(value string) standard.Name {
	result, err := standard.NewName(value)
	b.err = errors.Join(b.err, err)
	return result
}

func (b *standardBuilder) text(value string) standard.Text {
	result, err := standard.NewText(value)
	b.err = errors.Join(b.err, err)
	return result
}

func (b *standardBuilder) path(value string) standard.SourcePath {
	result, err := standard.ParseSourcePath(value)
	b.err = errors.Join(b.err, err)
	return result
}

func (b *standardBuilder) reason(title, detail string) standard.Reason {
	return standard.Reason{Title: b.name(title), Detail: b.text(detail)}
}

func (b *standardBuilder) boundary(title, detail string) standard.Boundary {
	return standard.Boundary{Title: b.name(title), Detail: b.text(detail)}
}

func TestStandardKnowledgeClosesAuthoredMeaning(t *testing.T) {
	t.Parallel()
	...
	changed := standard.GitOrigin{
		Commit: commit,
		At:     temporal.InstantFromNanoseconds(1_767_225_600_000_000_000),
	}
	knowledge, err := StandardKnowledge(standard.OptionalGitOrigin{}, changed)
	if err != nil {
		t.Fatalf("StandardKnowledge() error = %v, want nil", err)
	}
	wantPath, err := standard.ParseSourcePath(standardSourcePath)
	if err != nil {
		t.Fatalf("ParseSourcePath() error = %v, want nil", err)
	}
	if knowledge.Changed != changed || knowledge.SourcePath != wantPath {
		t.Fatalf("ProjectKnowledge changed/path = (%v, %v), want (%v, %v)", knowledge.Changed, knowledge.SourcePath, changed, wantPath)
	}
}
