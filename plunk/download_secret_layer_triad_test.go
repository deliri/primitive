...
func TestDownloadAndSecretCustodyLayerTriad(t *testing.T) {
	...
	t.Run("positive secret custody constructs independent redacted provider values", func(t *testing.T) {
		...
		credentialValue, err := secretstore.NewValue([]byte("sk_test_credential"))
		if err != nil {
			t.Fatalf("secretstore.NewValue(credential) error = %v, want nil", err)
		}
		webhookValue, err := secretstore.NewValue([]byte("verified-webhook-secret"))
		if err != nil {
	...
	t.Run("positive authenticated download streams the bounded response", func(t *testing.T) {
		...
		credential, credentialErr := ParseCredential([]byte("sk_test_credential"))
		...
		exchangeClient, clientErr := exchange.NewClient(&http.Client{Transport: idempotencyRoundTripFunc(func(request *http.Request) (*http.Response, error) {
			calls++
			if request.Method != http.MethodGet || request.URL.Host != core.PlunkAPIHost || request.Header.Get("Authorization") != "Bearer sk_test_credential" {
				return nil, core.ErrPlunkBinding
			}
			return &http.Response{StatusCode: http.StatusOK, Header: http.Header{"Content-Type": []string{"application/json"}}, Body: io.NopCloser(bytes.NewBufferString(`{"message":"delivered"}`)), Request: request}, nil
		})})
		if clientErr != nil {
		...
		request, destination := plunkDownloadFixture(t)
		if gotErr := request.Validate(); gotErr != nil {
			t.Fatalf("DownloadRequest.Validate() error = %v, want nil", gotErr)
		}
		response, gotErr := provider.Download(t.Context(), request, plunkStreamPolicy(t))
		if gotErr != nil || response.Validate() != nil || calls != 1 || destination.String() != `{"message":"delivered"}` {
	...
	t.Run("negative zero download never reaches transport", func(t *testing.T) {
		...
		t.Cleanup(func() { _ = provider.Close() })
		response, gotErr := provider.Download(t.Context(), DownloadRequest{}, plunkStreamPolicy(t))
		if !errors.Is(gotErr, core.ErrPlunkBinding) || response.Stream.Metadata.Attempts != 0 ||
			response.Stream.Metadata.Bytes.Uint64() != 0 || len(response.Stream.Metadata.Headers.Values) != 0 || calls != 0 {
			t.Fatalf("Client.Download(zero) = (%v, %v, calls:%d), want zero, errors.Is(..., %v), 0", response, gotErr, calls, core.ErrPlunkBinding)
		}
	})
}

func plunkDownloadFixture(t testing.TB) (DownloadRequest, *bytes.Buffer) {
	t.Helper()
	target, err := core.ParseHTTPEndpoint("https://" + core.PlunkAPIHost + "/v1/messages/message_123")
	if err != nil {
