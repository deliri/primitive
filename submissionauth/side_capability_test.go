...
func submissionAuthServer(t testing.TB, trusted attest.TrustedKeys) controlplane.Server {
	t.Helper()
	server, err := controlplane.NewServer(controlplane.ServerConfiguration{TrustedAuthorityKeys: trusted})
	if err != nil {
		t.Fatalf("controlplane.NewServer(submission authority) error = %v, want nil", err)
	}
	return server
}
