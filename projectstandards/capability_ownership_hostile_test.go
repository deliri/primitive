...
func TestResolveProjectRelationshipRejectsOwnershipContradictions(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "Primitive cannot claim an external adapter role", module: primitive, ownership: []CapabilityOwnership{adapterFilesystem}, wantErr: core.ErrProjectStandardsConflict},
		{name: "product cannot impersonate a Primitive implementation", module: product, ownership: []CapabilityOwnership{primitiveFilesystem}, wantErr: core.ErrProjectStandardsConflict},
		{name: "duplicated capability ownership is a contradiction", module: product, ownership: []CapabilityOwnership{adapterFilesystem, adapterFilesystem}, wantErr: core.ErrProjectStandardsConflict},
		{name: "unknown ownership role is rejected", module: product, ownership: []CapabilityOwnership{{Capability: filesystem}}, wantErr: core.ErrProjectStandardsContract},
		{name: "absent module identity is rejected", ownership: []CapabilityOwnership{adapterFilesystem}, wantErr: core.ErrProjectStandardsContract},
	}
...
func TestPackageFileCatalogValidateOwnershipPreservesMixedSiteEvidence(t *testing.T) {
	t.Parallel()
	...
	file := SourceFile{
		...
		Effects: SourceFileEffects{
			Capabilities:   []PrimitiveCapabilityUse{{Package: core.PackageFilestore}, {Package: core.PackageTemporal}, {Package: core.PackageExchange}},
			...
			Direct:         []SourceEffectSite{sourceEffectSiteFixture(t, &PrimitiveCapabilityUse{Package: core.PackageExchange}, 3)},
	...
	if gotErr := missing.ValidateOwnership(module, nil); !errors.Is(gotErr, core.ErrProjectStandardsConflict) {
		t.Fatalf("PackageFileCatalog.ValidateOwnership(unowned implementation) error = %v, want errors.Is(..., %v)", gotErr, core.ErrProjectStandardsConflict)
	}
}
