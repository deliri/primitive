	...
	t.Run("IOPS retains operation units", func(t *testing.T) {
		...
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.wantErr && !errors.Is(gotErr, core.ErrProjectStandardsContract) {
					t.Fatalf("NewMachineIOPS(%d) error = %v, want errors.Is(..., %v)", tc.value, gotErr, core.ErrProjectStandardsContract)
				}
			})
		}
	})
	...
	t.Run("byte throughput retains byte units", func(t *testing.T) {
		...
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.wantErr && !errors.Is(gotErr, core.ErrProjectStandardsContract) {
					t.Fatalf("NewMachineBytesPerSecond(%d) error = %v, want errors.Is(..., %v)", tc.value, gotErr, core.ErrProjectStandardsContract)
				}
			})
		}
	})
	...
	t.Run("bit throughput retains bit units", func(t *testing.T) {
		...
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.wantErr && !errors.Is(gotErr, core.ErrProjectStandardsContract) {
					t.Fatalf("NewMachineBitsPerSecond(%d) error = %v, want errors.Is(..., %v)", tc.value, gotErr, core.ErrProjectStandardsContract)
				}
			})
		}
	})
