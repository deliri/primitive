...
const machineProjectStandardsTestRoute = "/v1/project-standards/machine"
...
func TestMachineServiceAndSocketLayerTriad(t *testing.T) {
	...
	t.Run("negative repository cannot substitute another machine identity", func(t *testing.T) {
		...
		if !errors.Is(gotErr, core.ErrProjectStandardsTransport) || got.Metadata.Attempts != 0 || got.Response.SchemaVersion != 0 {
			t.Fatalf("MachineClient.Fetch(conflicting identity) = (%+v, %v), want zero and errors.Is(..., %v)", got, gotErr, core.ErrProjectStandardsTransport)
		}
...
func fixtureMachineHTTPServer(t testing.TB, repository MachineRepository) (*httptest.Server, exchange.SocketRoutePath) {
	t.Helper()
	path, pathErr := exchange.ParseSocketRoutePath(machineProjectStandardsTestRoute)
	if pathErr != nil {
		t.Fatalf("exchange.ParseSocketRoutePath(%q) setup error = %v, want nil", machineProjectStandardsTestRoute, pathErr)
	}
	...
	httpServer := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		if gotErr := server.Serve(writer, request); gotErr != nil {
			http.Error(writer, "project standards machine request refused", http.StatusInternalServerError)
		}
	}))
