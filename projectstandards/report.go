...
// projectstandards.md projection. The caller owns the destination and its lifecycle.
func WriteProjectMarkdown(destination io.Writer, project Project) error {
	if destination == nil {
		return contractError(errors.New("project standards project report destination is nil"))
	}
...
// projectstandards.md projection, including exact source and evidence references.
func WritePackageMarkdown(destination io.Writer, snapshot PackageSnapshot) error {
	if destination == nil {
		return contractError(errors.New("project standards package report destination is nil"))
	}
...
// before and after Project standards snapshots. It reports absence and negative movement
...
func WritePackageEvolutionMarkdown(destination io.Writer, evolution PackageEvolution) error {
	if destination == nil {
		return contractError(errors.New("project standards package evolution report destination is nil"))
	}
...
func writeEvolutionInventory(writer exactReportWriter, evolution PackageEvolution) error {
	...
	return writeFacts(writer, [][2]string{
		...
		{"Benchmarks", signed(change.Benchmarks)}, {"Fuzz targets", signed(change.FuzzTargets)},
	})
}
...
func writeEvolutionEvidence(writer exactReportWriter, evolution PackageEvolution) error {
	...
	return writeFacts(writer, [][2]string{
		{"Surfaces", signed(change.Surfaces)}, {"Requests", signed(change.Requests)},
		{"Admissions", signed(change.Admissions)}, {"Refusals", signed(change.Refusals)},
		{"Observations", signed(change.Observations)}, {"Current", signed(change.Current)},
		{"Stale", signed(change.Stale)}, {"Passed", signed(change.Passed)},
		{"Failed", signed(change.Failed)}, {"Non-accepting", signed(change.NonAccepting)},
		{"Benchmarks", signed(change.Benchmarks)}, {"Artifacts", signed(change.Artifacts)},
...
func writeEvolutionSourceUsage(writer exactReportWriter, evolution PackageEvolution) error {
	...
	return writeFacts(writer, [][2]string{
		...
		{"Declarations", signed(change.Declarations)}, {"Production referenced", signed(change.ProductionReferenced)},
		{"Runtime entry points", signed(change.RuntimeEntryPoints)}, {"Unresolved", signed(change.UnresolvedDeclarations)},
		{"Test only", signed(change.TestReferencedOnly)}, {"No reference observed", signed(change.NoReferenceObserved)},
...
func writeUsage(writer exactReportWriter, values []Usage) error {
	...
	for _, usage := range values {
		...
		if err := writeFacts(writer, [][2]string{{"Audience", usage.Audience.String()}, {"Goal", usage.Goal.String()}}); err != nil {
			return err
		}
...
func writeEvidence(writer exactReportWriter, snapshot PackageSnapshot) error {
	...
	return writeFacts(writer, [][2]string{
		{"Surfaces", decimal(summary.SurfaceCount)},
		{"Requests", decimal(summary.RequestedCount)},
		{"Admitted", decimal(summary.AdmittedCount)},
		{"Refused", decimal(summary.RefusedCount)},
		{"Observations", decimal(summary.ObservedCount)},
		...
		{"Passed", decimal(summary.PassedCount)},
		{"Failed", decimal(summary.FailedCount)},
		{"Non-accepting", decimal(summary.NonAcceptingCount)},
	})
}
...
func writeSourceUsage(writer exactReportWriter, usage *PackageSourceUsage) error {
	...
	return writeFacts(writer, [][2]string{
		{"Revision", usage.Revision.String()},
		{"Declarations", strconv.FormatUint(uint64(usage.DeclarationCount), 10)},
		{"Production referenced", strconv.FormatUint(uint64(usage.ProductionReferenced), 10)},
		{"Runtime entry points", strconv.FormatUint(uint64(usage.RuntimeEntryPoints), 10)},
		{"Unresolved", strconv.FormatUint(uint64(usage.UnresolvedDeclarations), 10)},
		{"Test only", strconv.FormatUint(uint64(usage.TestReferencedOnly), 10)},
		{"No reference observed", strconv.FormatUint(uint64(usage.NoReferenceObserved), 10)},
	})
}
