...
func TestPackageEvolutionMarkdownLayerTriad(t *testing.T) {
	...
	t.Run("positive report names both revisions and the newly observable function", func(t *testing.T) {
		...
		for _, want := range []string{before.Package.Revision.String(), after.Package.Revision.String(), "Before analysis:** unavailable", "After analysis:** available", "Newly observable.** projectstandards/evolution.go:unusedHelper"} {
	...
	t.Run("negative invalid evolution cannot cross the report boundary", func(t *testing.T) {
		...
		if !errors.Is(err, core.ErrProjectStandardsContract) || output.Len() != 0 {
			t.Fatalf("WritePackageEvolutionMarkdown(invalid) = (bytes %d, error %v), want (0, %v identity)", output.Len(), err, core.ErrProjectStandardsContract)
		}
	})
...
func TestPackageEvolutionLayerTriad(t *testing.T) {
	...
	t.Run("positive second Project standards exposes newly available source analysis and its exact review candidate", func(t *testing.T) {
		t.Parallel()
		before, after := evolutionFixture(t)
		...
		if len(got.FormerReviewCandidates) != 0 {
			t.Fatalf("former review candidates = %+v, want none when the first Project standards had no source analysis", got.FormerReviewCandidates)
		}
	})
	...
	t.Run("negative a different analyzer generation cannot masquerade as package evolution", func(t *testing.T) {
		...
		got, err := ComparePackageSnapshots(before, after)
		if !errors.Is(err, core.ErrProjectStandardsConflict) || len(got.NewReviewCandidates) != 0 || len(got.FormerReviewCandidates) != 0 {
			t.Fatalf("ComparePackageSnapshots(analyzer drift) = (%+v, %v), want (zero, %v identity)", got, err, core.ErrProjectStandardsConflict)
		}
	})

	t.Run("neutral identical Project standards snapshots produce zero movement and no invented review candidate", func(t *testing.T) {
		t.Parallel()
...
func TestPackageSnapshotRejectsSourceAnalysisFromAnotherRevision(t *testing.T) {
	...
	err := got.Validate()
	if !errors.Is(err, core.ErrProjectStandardsConflict) {
		t.Fatalf("PackageSnapshot.Validate(source analysis from another revision) error = %v, want %v identity", err, core.ErrProjectStandardsConflict)
	}
}
