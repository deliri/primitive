...
func TestMarkdownReporterLayerTriad(t *testing.T) {
	...
	t.Run("negative invalid record emits no report bytes", func(t *testing.T) {
		...
		if !errors.Is(gotErr, core.ErrProjectStandardsContract) || got.Len() != 0 {
			t.Fatalf("WritePackageMarkdown(invalid) = (%d bytes, %v), want (0, errors.Is(..., %v))", got.Len(), gotErr, core.ErrProjectStandardsContract)
		}
	})
