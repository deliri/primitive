...
func TestInventoryHostileBoundaryTable(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "one test file cannot claim one above the declaration ceiling", in: Inventory{Files: 1, TestFiles: 1, TestDeclarations: SourceFileDeclarationMaximum + 1}, wantErr: core.ErrProjectStandardsConflict},
		...
		{name: "test declaration count one above per-file ceiling is refused", in: Inventory{Files: 4, TestFiles: 4, TestDeclarations: 4*SourceFileDeclarationMaximum + 1}, wantErr: core.ErrProjectStandardsConflict},
