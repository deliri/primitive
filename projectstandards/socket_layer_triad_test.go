...
const projectStandardsTestRoute = "/v1/project-standards"

type fixtureProjectStandardsRepository struct {
	catalog      Catalog
	projectCalls atomic.Uint64
	packageCalls atomic.Uint64
}

func (r *fixtureProjectStandardsRepository) Project(context.Context, SubjectIdentity, core.BuildCommit) (Project, error) {
	r.projectCalls.Add(1)
	return r.catalog.Project, nil
}

func (r *fixtureProjectStandardsRepository) Package(context.Context, SubjectIdentity, core.BuildCommit, SourcePath) (PackageSnapshot, error) {
	r.packageCalls.Add(1)
	return r.catalog.Packages[0], nil
}

func TestProjectStandardsServiceAndSocketLayerTriad(t *testing.T) {
	t.Parallel()

	t.Run("positive real paired socket returns the exact project projection and execution metadata", func(t *testing.T) {
		t.Parallel()
		repository := &fixtureProjectStandardsRepository{catalog: fixtureCatalog(t)}
		server, path := fixtureProjectStandardsHTTPServer(t, repository)
		defer server.Close()
		client := fixtureProjectStandardsClient(t, server.Client(), server.URL, path)
	...
	t.Run("negative repository projection for another revision cannot cross the socket", func(t *testing.T) {
		...
		if !errors.Is(gotErr, core.ErrProjectStandardsTransport) || !gotIsZero {
			t.Fatalf("Client.Fetch(conflicting projection) = (%+v, %v), want (zero, errors.Is(..., %v))", got, gotErr, core.ErrProjectStandardsTransport)
		}
...
func fixtureProjectStandardsHTTPServer(t testing.TB, repository Repository) (*httptest.Server, exchange.SocketRoutePath) {
	t.Helper()
	path, pathErr := exchange.ParseSocketRoutePath(projectStandardsTestRoute)
	if pathErr != nil {
		t.Fatalf("exchange.ParseSocketRoutePath(%q) setup error = %v, want nil", projectStandardsTestRoute, pathErr)
	}
	...
	httpServer := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		if gotErr := server.Serve(writer, request); gotErr != nil {
			http.Error(writer, "project standards request refused", http.StatusInternalServerError)
		}
	}))
	return httpServer, path
}

func fixtureProjectStandardsClient(t testing.TB, httpClient *http.Client, baseURL string, path exchange.SocketRoutePath) Client {
	t.Helper()
